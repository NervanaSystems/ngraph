...
constexpr NodeTypeInfo op::v0::ArgMin::type_info;

op::v0::ArgMin::ArgMin(const Output<Node>& arg,
                       size_t axis,
...
bool op::v0::ArgMin::visit_attributes(AttributeVisitor& visitor)
{
    IndexReduction::visit_attributes(visitor);
    return true;
}

shared_ptr<Node> op::v0::ArgMin::clone_with_new_inputs(const OutputVector& new_args) const
{
    check_new_args_count(this, new_args);
    return make_shared<ArgMin>(new_args.at(0), m_axis, this->get_output_element_type(0));
}

std::shared_ptr<Node> op::v0::ArgMin::get_default_value() const
{
