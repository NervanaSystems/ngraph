...
shared_ptr<Node>
    op::Divide::copy_with_new_source_outputs(const OutputVector& new_source_outputs) const
{
    check_new_source_outputs_count(this, new_source_outputs);
    return make_shared<Divide>(new_source_outputs.at(0), new_source_outputs.at(1));
}

void op::Divide::generate_adjoints(autodiff::Adjoints& adjoints, const OutputVector& deltas)
{
    ...
    adjoints.add_output_delta(y, -delta * shared_from_this() / y);
}
