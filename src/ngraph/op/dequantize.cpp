...
op::Dequantize::Dequantize(const NodeOutput& input,
    ...
    : Op("Dequantize", {input, scale, offset})
    , m_type(type)
    , m_axes(axes)
{
...
shared_ptr<Node>
    op::Dequantize::copy_with_new_source_outputs(const OutputVector& new_source_outputs) const
{
    check_new_source_outputs_count(this, new_source_outputs);
    return make_shared<Dequantize>(new_source_outputs.at(0),
...
void op::Dequantize::generate_adjoints(autodiff::Adjoints& adjoints, const OutputVector& deltas)
{
