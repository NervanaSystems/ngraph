...
constexpr NodeTypeInfo op::v0::GreaterEqual::type_info;

op::v0::GreaterEqual::GreaterEqual(const Output<Node>& arg0,
                                   const Output<Node>& arg1,
                                   const AutoBroadcastSpec& auto_broadcast)
    : BinaryElementwiseComparison(arg0, arg1, auto_broadcast)
{
...
shared_ptr<Node> op::v0::GreaterEqual::clone_with_new_inputs(const OutputVector& new_args) const
{
    check_new_args_count(this, new_args);
    return make_shared<op::v0::GreaterEqual>(new_args.at(0), new_args.at(1), this->get_autob());
}

bool op::v0::GreaterEqual::evaluate(const HostTensorVector& outputs,
                                    const HostTensorVector& inputs) const
{
    return evaluate_greater_equal(inputs[0], inputs[1], outputs[0], get_autob());
}
