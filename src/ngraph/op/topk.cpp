...
op::v3::TopK::TopK(const Output<Node>& data,
                   ...
                   const std::string& mode,
                   const std::string& sort,
                   const element::Type& output_indices_type)
    : Op{{data, k}}
    , m_axis{axis}
    , m_normalized_axis{0}
...
void op::v3::TopK::validate_and_infer_types()
{
    ...
    set_output_size(2);
    set_output_type(0, get_input_element_type(0), output_shape);
    set_output_type(1, m_output_indices_type, output_shape);
}

void op::v3::TopK::set_axis(const int64_t axis)
{
    const auto input_rank = get_input_partial_shape(0).rank();
    if (input_rank.is_static())
    {
...
uint64_t op::v3::TopK::get_axis() const
{
    NODE_VALIDATION_CHECK(
        this, m_normalized_axis != UNKNOWN_NORMALIZED_AXIS, "Normalized axis of TopK is unknown");
...
template <typename T>
size_t op::v3::TopK::validate_and_get_k(const shared_ptr<op::Constant>& k_constant) const
{
    const auto k_const_contents = k_constant->get_vector<T>();
...
void op::v3::TopK::generate_adjoints(autodiff::Adjoints& /*adjoints*/,
                                     const OutputVector& /* deltas */)
{
    throw ngraph_error("Forward-propagation-only operation");
}
...
void op::v3::TopK::set_k(size_t k)
{
    this->input(1).replace_source_output(
        op::Constant::create(element::i64, Shape{}, {k})->output(0));
}
