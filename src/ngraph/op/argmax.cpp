...
op::ArgMax::ArgMax(const Output<Node>& arg, size_t axis, element::Type index_element_type)
...
shared_ptr<Node> op::ArgMax::clone_with_new_inputs(const OutputVector& new_args) const
{
    ...
    return make_shared<ArgMax>(new_args.at(0), m_axis, this->get_output_element_type(0));
}
