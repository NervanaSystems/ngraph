    ...
    namespace op
    {
        namespace detail
        {
            ...
            std::shared_ptr<Node> lp_norm(const std::shared_ptr<Node>& node,
                                          ...
                                          float bias)
            {
                std::shared_ptr<Node> abs_values{std::make_shared<Abs>(node)};
                std::shared_ptr<Node> p_node = Constant::create(
                    node->get_element_type(),
                    node->get_shape(),
                    std::vector<float>(shape_size(node->get_shape()), static_cast<float>(p_norm)));

                std::shared_ptr<Node> values{std::make_shared<Power>(abs_values, p_node)};

                values = std::make_shared<Sum>(values, reduction_axes);

                std::shared_ptr<Node> bias_node{
                    detail::get_bias_node(values->get_element_type(), values->get_shape(), bias)};
                values = values + bias_node;

                std::shared_ptr<Node> inv_p_node = Constant::create(
                    values->get_element_type(),
                    values->get_shape(),
                    std::vector<float>(shape_size(values->get_shape()), 1.f / p_norm));

                return {std::make_shared<Power>(values, inv_p_node)};
            }
        }
                                      ...
                                      const AxisSet& reduction_axes)
        {
            ...
            std::shared_ptr<Node> zero_node{
                Constant::create(node->get_element_type(),
                                 node->get_shape(),
                                 std::vector<float>(shape_size(node->get_shape()), 0.f))};

            std::shared_ptr<Node> non_zero_values = std::make_shared<Convert>(
                std::make_shared<NotEqual>(abs_values, zero_node), abs_values->get_element_type());

            return std::make_shared<Sum>(non_zero_values, reduction_axes);
        }
            ...
            l1_norm(const std::shared_ptr<Node>& node, const AxisSet& reduction_axes, float bias)
        {
            std::shared_ptr<Node> values{
                std::make_shared<Sum>(std::make_shared<Abs>(node), reduction_axes)};
            ...
            l2_norm(const std::shared_ptr<Node>& node, const AxisSet& reduction_axes, float bias)
        {
            std::shared_ptr<Node> abs_values{std::make_shared<Abs>(node)};
            std::shared_ptr<Node> values{
                std::make_shared<Sum>(abs_values * abs_values, reduction_axes)};

            std::shared_ptr<Node> bias_node{
                detail::get_bias_node(values->get_element_type(), values->get_shape(), bias)};

            return {std::make_shared<Sqrt>(values + bias_node)};
        }
