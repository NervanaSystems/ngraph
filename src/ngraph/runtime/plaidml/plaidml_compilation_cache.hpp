...
{
public:
    // Looks up the supplied function in the compilation cache.  If the function is not in the cache, returns an empty pointer.
    std::shared_ptr<CompiledFunction> try_lookup(std::shared_ptr<Function> func);

    // Looks up the supplied function in the compilation cache.  If the function is not in the cache, compiles it using the
    // specified compiler (which must not be nullptr), adds the compiled function to the cache, and returns the compiled function.
...
private:
    std::mutex mu_;
    std::unordered_map<std::shared_ptr<Function>, std::shared_ptr<CompiledFunction>> cache_;
};
