    ...
    namespace runtime
    {
        namespace plaidml
        {
                                     ...
                                     .set(builder::ContractionOutput{"O"}
                                              .add_indices([&](
                                                  std::back_insert_iterator<std::list<std::string>>
                                                      out) {
                                                  for (std::size_t idx = 0; idx < out_idxs.size();
                                                       ++idx)
                                                  {
            ...
            void Impl<op::Reduce>::operator()()
            {
                ...
                // To support arbitrary reduction operations, we take advantage of the fact that in nGraph,
                // we have concrete dimension sizes.  We start with the initial tensor (argument 1),
                // construct N slices of tensor 0 (where N == the product of the sizes of the axes to reduce),
                // and repeatedly apply the supplied aggregation function to them.
                //
                ...
                // Note that we need to explicitly broadcast the 0-dimensional base result to match the aggregation dimension count.
                vp::variable result =
                    start_tile_function()
