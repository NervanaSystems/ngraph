    ...
    namespace runtime
    {
        namespace plaidml
        {
            ...
            void Impl<op::OneHot>::operator()()
            {
                ...
                //
                // * We reshape the input tensor to add a size=1
                //   dimension where we want the one-hot axis to be,
                //
                // * We create an index tensor that's size=1 on every
                //   dimension except the one-hot dimension,
                //
                // * We perform an elementwise conditional across
                //   them to assign the one-hot values.
                //
                // The broadcast rules will expand the index tensor on
                // all non-one-hot dimensions to match the input, and
                // will expand the input tensor on the one-hot
                // dimension to match the index.
                //
                // In theory, it'd be pretty easy to implement all
                // this with purely elementwise operations.  The
                // current definition of index() requires an input
                // tensor of the index() output shape, and it's a
                // little tricky to fix that, so we generate a zero
                // tensor of the correct shape using a contraction.
                // TODO: Optimize out the zero tensor contraction.
