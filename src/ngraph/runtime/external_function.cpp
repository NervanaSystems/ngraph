...
#include "ngraph/runtime/eigen/reduce_matrix_columns.hpp"
#include "ngraph/runtime/eigen/reduce_matrix_rows.hpp"
...
#undef REGISTER_CONVERT
        ...
        REGISTER_TO_OP_MAP(op::FunctionCall)
        {
            ...
            {
                external = make_shared<ngraph::runtime::ExternalFunction>(function);
        ...
        REGISTER_TO_OP_MAP(op::Reduce)
        {
            ...
            {
                external = make_shared<ngraph::runtime::ExternalFunction>(reduction_function);
            ...
            else if (reductee_shape.size() == 2 && reduction_axes == AxisSet{1})
            {
                ...
                else
                {
                    PUSH_POLYMORPHIC_INSTRUCTION(f_result_element_type,
                                                 "Reduce has unhandled element type",
                                                 runtime::eigen::ReduceMatrixRowsInstruction,
                                                 external,
                                                 in[0],
                                                 in[1],
                                                 out[0]);
            ...
            else if (reductee_shape.size() == 2 && reduction_axes == AxisSet{0})
            {
                ...
                else
                {
                    PUSH_POLYMORPHIC_INSTRUCTION(f_result_element_type,
                                                 "Reduce has unhandled element type",
                                                 runtime::eigen::ReduceMatrixColumnsInstruction,
                                                 external,
                                                 in[0],
                                                 in[1],
                                                 out[0]);
