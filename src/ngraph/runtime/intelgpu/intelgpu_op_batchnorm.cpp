                                           ...
                                           const std::string& mean_name)
{
    if (input_shape.size() < 2 || input_shape.size() > 4)
    {
        throw invalid_argument("intelgpu::do_create_variance() wrong input shapes.");
    }
...
void runtime::intelgpu::do_create_variance_back(cldnn::topology& topology,
                                                const string& output_name,
                                                const Shape& small_shape,
                                                const element::Type& output_type,
                                                double eps,
                                                const string& input_name,
                                                const Shape& big_shape,
                                                const string& mean_name,
                                                const string& variance_name,
                                                const string& delta_name)
{
    const size_t channel_axis = 1;
    const cldnn::layout layout = IntelGPULayout::create_cldnn_layout(output_type, small_shape);
    const string entry_point_name = "create_variance_back_" + output_name;
    codegen::CodeWriter writer;

    writer << "__kernel void " << entry_point_name << "(const __global float input"
           << array_dims(big_shape) << ", const __global float delta" << array_dims(big_shape)
           << ", const __global float mean" << array_dims(small_shape)
           << ", const __global float variance" << array_dims(small_shape)
           << ", __global float output" << array_dims(small_shape) << ")\n";
    ...
    { // Main function body
        ...
        // Loop for Channel axis 1
        writer << "for (uint i" << channel_axis << " = 0; i" << channel_axis << " < "
               << big_shape.at(channel_axis) << "; ++i" << channel_axis << ")\n";
        writer.block_begin();
        {
            writer << "float sum = 0.0f;\n";
            ...
            writer << "float normalized = (input" << access_dims(big_shape) << " - mean[i"
                   << channel_axis << "]) / ("
                   << "sqrt(variance[i" << channel_axis << "] + " << eps << ")"
                   << ");\n";

            writer << "sum += normalized * delta" << access_dims(big_shape) << ";\n";
...
{
    const size_t channel_axis = 1;
    const Shape channel_shape = {shape.at(channel_axis)};
