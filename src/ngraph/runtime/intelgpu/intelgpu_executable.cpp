...
#pragma GCC diagnostic pop
    ...
    ocl_network = make_shared<cldnn::network>(*ocl_engine, topology, network_build_options);
}

bool runtime::intelgpu::IntelGPUExecutable::execute(const vector<runtime::Tensor*>& outputs,
                                                    const vector<runtime::Tensor*>& inputs)
{
    ...
    shared_ptr<cldnn::network> network = ocl_network;
    ...
    {
        const shared_ptr<Node>& dst_node = get_results()[i];
        ...
        memory_size_check(result_memory.size(), dst_node, m_function_name);
    ...
    runtime::intelgpu::IntelGPUExecutable::get_performance_data() const
{
    ...
    const shared_ptr<cldnn::network> network = ocl_network;

    if (network != nullptr && m_performance_counters_enabled)
    {
    ...
    double mem_after_call) const
{
    ...
    const string& func_name = m_function_name;
    ...
    if (m_profile_lines_limit_count > 0)
    {
        ...
        {
            const string ngraph_node_name = convert_cldnn_names(m_function, it->second.item_name);
            const Node* ngraph_node = get_node_by_name(m_function, ngraph_node_name);
        ...
        const string total_items_count_string = "Total(cldnn " + to_string(total_items_count) +
                                                ", ngraph " +
                                                to_string(m_function->get_ops().size()) + ")";
