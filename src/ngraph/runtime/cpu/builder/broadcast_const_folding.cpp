...
namespace ngraph
{
    namespace runtime
    {
        ...
        {
            template <>
            CFFunctionTy Builder::BUILDER_CF_DECL(ngraph::op::Broadcast)
            {
                ...
                {
                    size_t size = shape_size(out_shape) * broadcast->get_element_type().size();
                    auto functor = [size](const std::vector<void*> inputs,
