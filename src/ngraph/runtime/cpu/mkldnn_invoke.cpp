...
extern "C" void ngraph::runtime::cpu::mkldnn_utils::mkldnn_invoke_primitive_new(
    CPURuntimeContext* ctx, size_t primitive_index, std::vector<size_t>& deps, OpType type, mkldnn::memory& scratchpad)
{
    std::unordered_map<int, mkldnn::memory> exec_args;
    size_t nargs;
    switch (type)
