    ...
    namespace runtime
    {
        namespace cpu
        {
            namespace kernel
            {
                ...
                void
                    merge(void* cond, void* tval, void* fval, void* output, size_t count, int arena)
                {
                    ...
                    if (static_cast<char*>(cond)[0])
                    {
                        Eigen::TensorMap<Eigen::Tensor<ElementType, 1, Eigen::RowMajor>> in(
                            static_cast<ElementType*>(tval), in_dims);
                    ...
                    {
                        Eigen::TensorMap<Eigen::Tensor<ElementType, 1, Eigen::RowMajor>> in(
                            static_cast<ElementType*>(fval), in_dims);
                        out.device(ngraph::runtime::cpu::executor::GetCPUExecutor().get_device(
                            arena)) = in;
                    }
                }
