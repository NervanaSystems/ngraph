...
void ngraph::runtime::cpu::pass::CPUInceptionHorizontalFusion::cpu_inception_horizontal_fusion()
{
    ...
    pattern::graph_rewrite_callback callback = [data_conv](pattern::Matcher& m) {
        ...
        for (auto u : m.get_pattern_map()[data_conv]->get_users())
        {
            ...
            {
                //NGRAPH_DEBUG << "inception: not CBR node\n";
                if (pattern::has_class<ngraph::op::AvgPool>()(u))
                {
                    NGRAPH_DEBUG << "inception: avg_pool node\n";
                }
                else if (pattern::has_class<ngraph::op::MaxPool>()(u))
                {
                    NGRAPH_DEBUG << "inception: max_pool node\n";
                }
                else
                {
                    NGRAPH_DEBUG << "inception: third type?\n";
                }
                continue;
            }
            NGRAPH_DEBUG << "inception: CBR node\n";
            auto u_filters_shape = u->get_input_shape(1);
            //NGRAPH_DEBUG << u_filters_shape;
        ...
        for (auto cb : conv_bias_nodes)
        {
            ...
            NGRAPH_DEBUG << "inception: cb shape " << slice_shape << "\n";
