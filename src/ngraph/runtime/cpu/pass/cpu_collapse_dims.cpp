...
static bool collapse_broadcast(std::shared_ptr<Node> n)
{
    ...
    auto input_shape = node->input_value(0).get_shape();
    ...
    if (cshape.axis_set.size() == 0)
    {
        ...
        auto reshape =
            std::make_shared<op::Reshape>(node->input_value(0), axis_order, n->get_shape());
...
static bool collapse_dot(std::shared_ptr<Node> n)
{
    bool replaced = false;
    auto node = std::static_pointer_cast<T>(n).get();
    auto A_shape = node->input_value(0).get_shape();
    auto B_shape = node->input_value(1).get_shape();
