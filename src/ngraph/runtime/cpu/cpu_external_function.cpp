...
using namespace ngraph::runtime;
        ...
        */
                ...
                if (!node->is_parameter() && !node->is_constant())
                {
                    ...
                    auto computes_output = [&]() {
                        ...
                        for (const descriptor::Output& output : node->get_outputs())
                        {
                            ...
                            {
                                auto res = std::dynamic_pointer_cast<ngraph::op::Result>(
                                    input->get_node());
            ...
            if (m_use_tbb)
            {
                ...
                traverse_nodes(current_function,
                               [&writer, &dependence_graph_heads](shared_ptr<Node> n) {
    ...
    if (m_direct_execution)
    {
        ...
        for (shared_ptr<Node> node : m_function->get_ordered_ops())
        {
            ...
            handler->second(this, w, node.get(), in, out);
        }
