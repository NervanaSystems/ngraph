...
namespace ngraph
{
    namespace runtime
    {
        namespace cpu
        {
            ...
            template <typename T>
            void emitBatchDot(const ngraph::Node* node,
                ...
                codegen::CodeWriter& writer)
            {
                ...
                auto mat_b = args[1];
                auto mat_c = out[0];
                const Shape& shape_a = batch_dot->get_a_shape();
                const Shape& shape_b = batch_dot->get_b_shape();
                const Shape& shape_c = mat_c.get_shape();
                ...
                const size_t offset_a = (shape_a.size() == 3) ? m * k : 0;
                const size_t offset_b = (shape_b.size() == 3) ? k * n : 0;
                const size_t offset_c = (shape_c.size() == 3) ? m * n  : 0;
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::MatmulBias)
            {
                ...
                const Shape& arg0_shape = cg->get_a_shape(); //W
                const Shape& arg1_shape = cg->get_b_shape(); //x
                const Shape& arg2_shape = node->get_shape();    //bias (C)

                //special case rank 3 by rank 2
                if (arg0_shape.size() == 3)
                {
                    auto bias = args[2];
                    kernel::emit_broadcast(writer,
                                       bias.get_element_type().c_type_string(),
                                       bias.get_name(),
                                       out[0].get_name(),
                                       bias.get_shape(),
                                       out[0].get_shape(),
                                       cg->get_broadcast_axes());
                    emitBatchDot<ngraph::op::MatmulBias>(node, args, out, writer);
                    return;
                }
            ...
            {
                emitBatchDot<ngraph::op::BatchDot>(node, args, out, writer);
            }
