...
mkldnn::memory::desc
    MKLDNNEmitter::build_memory_descriptor(const ngraph::Shape& shape,
                                           ...
                                           const int mask)
{
    ...
    auto reorder_prim_desc = mkldnn::reorder::primitive_desc(
        executor::global_cpu_engine, input_desc, executor::global_cpu_engine, result_desc, attr);
    mkldnn_primitives[quantize_index] = new mkldnn::reorder(reorder_desc);
}
    ...
    const mkldnn::memory::desc& weights_desc)
{
    ...
    build_memory(mkldnn_memories, deconv_desc.data.src_desc, delta_index);
    size_t bias_index = deps[2];
    build_memory(mkldnn_memories, deconv_desc.data.bias_desc, bias_index);
    size_t result_index = deps[3];
    build_memory(mkldnn_memories, deconv_desc.data.dst_desc, result_index);
    ...
    size_t conv_index)
{
    ...
    build_memory(mkldnn_memories, bwd_desc.data.src_desc, src_index);
    size_t diff_dst_index = deps[1];
    build_memory(mkldnn_memories, bwd_desc.data.diff_dst_desc, diff_dst_index);
    size_t diff_weights_index = deps[2];
    build_memory(mkldnn_memories, bwd_desc.data.diff_weights_desc, diff_weights_index);
    size_t diff_bias_index = deps[3];
    build_memory(mkldnn_memories, bwd_desc.data.diff_bias_desc, diff_bias_index);
    ...
    mkldnn_primitives[conv_index] = new mkldnn::convolution_backward_weights(bwd_pd);
}
...
{
    size_t weights_index = deps[0];
    build_memory(mkldnn_memories, bwd_desc.data.weights_desc, weights_index);
    size_t diff_dst_index = deps[1];
    build_memory(mkldnn_memories, bwd_desc.data.diff_dst_desc, diff_dst_index);
    size_t diff_src_index = deps[2];
    build_memory(mkldnn_memories, bwd_desc.data.diff_src_desc, diff_src_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, pool_desc.data.src_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, pool_desc.data.dst_desc, result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, pool_desc.data.diff_dst_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, pool_desc.data.diff_src_desc, result_index);
    ...
    mkldnn_primitives[pool_index] = new mkldnn::pooling_backward(pool_pd);
}
                                               ...
                                               size_t bwd_pool_index)
{
    ...
    build_memory(mkldnn_memories, bwd_pool_desc.data.diff_dst_desc, diff_dst_index);
    size_t diff_src_index = fdeps[1];
    build_memory(mkldnn_memories, bwd_pool_desc.data.diff_src_desc, diff_src_index);
    ...
    build_memory(mkldnn_memories, fwd_pd.workspace_primitive_desc().desc(), ws_index);
    bdeps[1] = ws_index;
    ...
    mkldnn_primitives[fwd_pool_index] = new mkldnn::pooling_forward(fwd_pd);
...
{
    size_t src_index = deps[0];
    build_memory(mkldnn_memories, max_pool_desc.data.src_desc, src_index);
    size_t dst_index = deps[1];
    build_memory(mkldnn_memories, max_pool_desc.data.dst_desc, dst_index);
    ...
    size_t max_pool_index)
{
    ...
    build_memory(mkldnn_memories, fwd_pd.workspace_primitive_desc().desc(), fprop_ws_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, lrn_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, lrn_desc.data.data_desc, result_index);

    auto lrn_prim_desc = mkldnn::lrn_forward::primitive_desc(lrn_desc, executor::global_cpu_engine);

    mkldnn_primitives[lrn_index] = new mkldnn::lrn_forward(lrn_prim_desc);
}
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, relu_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, relu_desc.data.data_desc, result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, bwd_desc.data.data_desc, input_index);
    size_t delta_index = deps[1];
    build_memory(mkldnn_memories, bwd_desc.data.diff_data_desc, delta_index);
    size_t result_index = deps[2];
    build_memory(mkldnn_memories, bwd_desc.data.data_desc, result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, sigmoid_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, sigmoid_desc.data.data_desc, result_index);
...
void MKLDNNEmitter::build_elementwise_add(std::vector<mkldnn::memory*>& mkldnn_memories,
                                          ...
                                          size_t add_index)
{
    build_memory(mkldnn_memories, sum_pd.dst_primitive_desc().desc(), input0_data_index);
    size_t input1_data_index = deps[1];
    build_memory(mkldnn_memories, sum_pd.dst_primitive_desc().desc(), input1_data_index);
    size_t result_index = deps[2];
    build_memory(mkldnn_memories, sum_pd.dst_primitive_desc().desc(), result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, batchnorm_desc.data.data_desc, input_index);
    ...
    if (bn_training_flag && !use_global_stats)
    {
        ...
        build_memory(mkldnn_memories, weights_desc, weights_index);
        size_t result_index = deps[2];
        build_memory(mkldnn_memories, batchnorm_desc.data.data_desc, result_index);
        size_t mean_index = deps[3];
        build_memory(mkldnn_memories, batchnorm_desc.data.mean_desc, mean_index);
        size_t variance_index = deps[4];
        build_memory(mkldnn_memories, batchnorm_desc.data.variance_desc, variance_index);
    ...
    size_t batchnorm_index)
{
    ...
    build_memory(mkldnn_mmeories, batchnorm_desc.data.variance_desc, variance_index);
    size_t delta_index = deps[4];
    build_memory(mkldnn_memories, batchnorm_desc.data.diff_data_desc, delta_index);
    size_t dinput_index = deps[5];
    build_memory(mkldnn_memories, batchnorm_desc.data.data_desc, dinput_index);
                                 ...
                                 size_t concat_index)
{
    ...
    build_memory(mkldnn_memories, concat_pd.dst_primitive_desc().desc(), result_index);
                                ...
                                size_t slice_index)
{
    ...
    auto input_sub_desc = mkldnn::desc(input_desc, dims, offsets);
    size_t input_index = deps[0];
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, softmax_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, softmax_desc.data.data_desc, result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, leaky_relu_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, leaky_relu_desc.data.data_desc, result_index);
...
{
    size_t input_index = deps[0];
    build_memory(mkldnn_memories, bounded_relu_desc.data.data_desc, input_index);
    size_t result_index = deps[1];
    build_memory(mkldnn_memories, bounded_relu_desc.data.data_desc, result_index);
