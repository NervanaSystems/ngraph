...
std::string runtime::nvgpu::NVExternalFunction::add_to_runtime(
    ...
    const std::vector<runtime::nvgpu::NVTensorWrapper>& out)
{
    ...
    writer.block_begin();
    {
        ...
        writer << "nvgpu::invoke_primitive(ctx, " << primitive_index << ", input, output);\n";
        writer.block_end();
    }
    return writer.get_code();
}

std::string runtime::nvgpu::NVExternalFunction::add_call_to_runtime(
...
const string& runtime::nvgpu::NVExternalFunction::get_header_source()
{
...
using namespace std;
        ...
        + "void *__dso_handle = 0;\n\n" +
        "static nvgpu::NVRuntimeContext* m_runtime_context = nullptr;\n";

    return s_header_source;
}

void runtime::nvgpu::NVExternalFunction::emit_header()
{
    m_writer << get_header_source();
}

void runtime::nvgpu::NVExternalFunction::emit_timer_functions()
{
    if (m_emit_timing)
    {
        ...
        m_writer << "return (index < " << names.size()
                 << " ? runtime::nvgpu::us_stopwatch(m_runtime_context, index + " << m_offset
                 << ") : 0);\n";
        m_writer.block_end();
        ...
        m_writer.block_begin();
        m_writer << "return (index < " << names.size()
                 << " ? runtime::nvgpu::count_stopwatch(m_runtime_context, index + " << m_offset
                 << ") : 0);\n";
        m_writer.block_end();
        m_writer << "\n";
    }
...
void runtime::nvgpu::NVExternalFunction::emit_constant_declarations()
{
    ...
    for (const auto& p : m_function_ordered_ops)
    {
        for (shared_ptr<Node> node : p.second)
        {
            ...
            if (c)
            {
                ...
                runtime::nvgpu::NVAllocator allocator =
                ...
                for (shared_ptr<Node> node : p.second)
                {
                    ...
                    if (c)
                    {
                        ...
                        m_writer << tv->get_name() << " = reinterpret_cast<"
                                 ...
                                 << "*>(runtime::nvgpu::invoke_memory_primitive(m_runtime_context, "
                                 << tv->get_name() << "_idx));\n";
                    }
                }
            }
...
void runtime::nvgpu::NVExternalFunction::emit_function_declarations()
{
    m_writer << "// Declare all functions\n";
    for (const auto& p : m_function_ordered_ops)
    {
        m_writer << "extern \"C\" void " << p.first->get_name() << "(void** inputs, "
                 << "void** outputs, nvgpu::NVRuntimeContext* ctx);\n";
    }
    m_writer << "\n";
}

void runtime::nvgpu::NVExternalFunction::emit_temp_mem_pool_allocation(
    shared_ptr<Function> current_function)
{
    ...
    if (temporaries_used)
    {
        ...
        m_writer
            << "char* pool_base_ptr = (char*)ngraph::runtime::nvgpu::invoke_memory_primitive(ctx, "
...
void runtime::nvgpu::NVExternalFunction::emit_functions()
{
    for (const auto& p : m_function_ordered_ops)
    {
        ...
        m_writer << "(void** inputs, void** outputs, "
                 << "nvgpu::NVRuntimeContext* ctx) __attribute__ ((optnone))\n";
        m_writer.block_begin();
        {
...
void runtime::nvgpu::NVExternalFunction::store_emitted_functions(const string& code)
{
...
void runtime::nvgpu::NVExternalFunction::add_passes(ngraph::pass::Manager& pass_manager)
{
    auto femitter = bind(&ngraph::runtime::nvgpu::NVExternalFunction::emit_op_as_function,
                         this,
                         placeholders::_1,
                         placeholders::_2);
...
void runtime::nvgpu::NVExternalFunction::emit()
{
    emit_header();
...
void runtime::nvgpu::NVExternalFunction::compile_function()
{
...
void runtime::nvgpu::NVExternalFunction::emit_debug_function_entry(Node* node)
{
    if (m_emit_timing)
    {
        m_writer << "runtime::nvgpu::start_stopwatch(ctx, "
                 << m_name_index_map[node->get_name()] + m_offset << ");\n";
    }
}

void runtime::nvgpu::NVExternalFunction::emit_debug_function_exit(Node* node)
{
    if (m_emit_timing)
    {
        m_writer << "runtime::nvgpu::stop_stopwatch(ctx, "
                 << m_name_index_map[node->get_name()] + m_offset << ");\n";
    }
}
...
void runtime::nvgpu::NVExternalFunction::propagate_in_place_output(
...
string runtime::nvgpu::NVExternalFunction::strip_comments(const string& s) const
{
    stringstream out;
    for (size_t i = 0; i < s.size(); i++)
    {
...
void runtime::nvgpu::NVExternalFunction::get_performance_data(
