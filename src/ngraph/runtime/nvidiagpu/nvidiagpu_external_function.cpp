...
std::string runtime::nvidiagpu::NVExternalFunction::emit_op(NVCompiledFunction* external_function,
...
runtime::nvidiagpu::NVExternalFunction::NVExternalFunction(
...
std::string runtime::nvidiagpu::NVExternalFunction::add_to_runtime(
    ...
    const std::vector<runtime::nvidiagpu::NVTensorWrapper>& out)
{
    ...
    writer.block_begin();
    {
        ...
        writer << "nvidiagpu::invoke_primitive(ctx, " << primitive_index << ", input, output);\n";
        writer.block_end();
    }
    return writer.get_code();
}

std::string runtime::nvidiagpu::NVExternalFunction::add_call_to_runtime(
...
std::string runtime::nvidiagpu::NVExternalFunction::node_names(
    const std::vector<runtime::nvidiagpu::NVTensorWrapper>& args,
...
const string& runtime::nvidiagpu::NVExternalFunction::get_pch_header_source()
{
    static string s_pch_header_source = R"(
// Generated by the nGraph NV backend
#include <cublas_v2.h>
#include <cuda.h>
#include <cuda_runtime.h>
#include <cudnn.h>
...
const string& runtime::nvidiagpu::NVExternalFunction::get_header_source()
{
...
void runtime::nvidiagpu::NVExternalFunction::emit_header()
{
    m_writer << get_header_source();
}

void runtime::nvidiagpu::NVExternalFunction::emit_timer_functions()
{
    if (m_emit_timing)
    {
        ...
        m_writer << "return (index < " << names.size()
                 << " ? runtime::nvidiagpu::us_stopwatch(m_runtime_context, index + " << m_offset
                 << ") : 0);\n";
        m_writer.block_end();
        ...
        m_writer.block_begin();
        m_writer << "return (index < " << names.size()
                 << " ? runtime::nvidiagpu::count_stopwatch(m_runtime_context, index + " << m_offset
                 << ") : 0);\n";
        m_writer.block_end();
        m_writer << "\n";
    }
...
void runtime::nvidiagpu::NVExternalFunction::emit_constant_declarations()
{
...
void runtime::nvidiagpu::NVExternalFunction::emit_function_declarations()
{
...
void runtime::nvidiagpu::NVExternalFunction::emit_temp_mem_pool_allocation(
...
void runtime::nvidiagpu::NVExternalFunction::emit_functions()
{
    for (const auto& p : m_function_ordered_ops)
    {
...
void runtime::nvidiagpu::NVExternalFunction::store_emitted_functions(const string& code)
{
...
void runtime::nvidiagpu::NVExternalFunction::add_passes(ngraph::pass::Manager& pass_manager)
{
    auto femitter = bind(&ngraph::runtime::nvidiagpu::NVExternalFunction::emit_op_as_function,
                         this,
                         placeholders::_1,
                         placeholders::_2);
...
void runtime::nvidiagpu::NVExternalFunction::emit()
{
    emit_header();
...
void runtime::nvidiagpu::NVExternalFunction::compile_function()
{
...
void runtime::nvidiagpu::NVExternalFunction::emit_debug_function_entry(Node* node)
{
    if (m_emit_timing)
    {
        m_writer << "runtime::nvidiagpu::start_stopwatch(ctx, "
                 << m_name_index_map[node->get_name()] + m_offset << ");\n";
    }
}

void runtime::nvidiagpu::NVExternalFunction::emit_debug_function_exit(Node* node)
{
    if (m_emit_timing)
    {
        m_writer << "runtime::nvidiagpu::stop_stopwatch(ctx, "
                 << m_name_index_map[node->get_name()] + m_offset << ");\n";
    }
}

string runtime::nvidiagpu::NVExternalFunction::emit_op_as_function(const Node& node,
...
void runtime::nvidiagpu::NVExternalFunction::propagate_in_place_input(
...
void runtime::nvidiagpu::NVExternalFunction::propagate_in_place_output(
...
string runtime::nvidiagpu::NVExternalFunction::strip_comments(const string& s) const
{
    stringstream out;
    for (size_t i = 0; i < s.size(); i++)
    {
...
void runtime::nvidiagpu::NVExternalFunction::get_performance_data(
