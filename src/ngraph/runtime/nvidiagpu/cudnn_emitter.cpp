...
std::vector<int> runtime::nvidiagpu::cudnn_util::compute_strides(const std::vector<int>& shape)
{
...
std::vector<int>
    runtime::nvidiagpu::cudnn_util::get_vector_int_from_size_t(const std::vector<size_t>& vec)
{
...
cudnnDataType_t runtime::nvidiagpu::CUDNNEmitter::get_cudnn_datatype(std::string dtype)
{
...
cudnnDataType_t runtime::nvidiagpu::CUDNNEmitter::get_cudnn_datatype(const element::Type& dtype)
{
        ...
        if (output_type == element::i32 || output_type == element::i64)
        {
            ...
            if (input_type == element::i32)
            {
                ...
                convert_input = [=](void** inputs, void** outputs) {
                    nvidiagpu::invoke_primitive(m_ctx, convert_input_idx, inputs, outputs);
                };
                convert_input_space = [=](void* ptr) {
                    return runtime::nvidiagpu::invoke_memory_primitive(m_ctx, input_idx);
                };
            }
                                                         ...
                                                         const double beta)
{
    ...
    std::unique_ptr<nvidiagpu::primitive> tensor(
        new nvidiagpu::primitive{[=, &opTensorDesc, &descriptor](void** inputs, void** outputs) {
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::Convolution* node)
{
    ...
    ngraph::Shape padding_below(padding_below_diff.size(), 0);
    ngraph::Shape padding_above(padding_above_diff.size(), 0);
    ...
    ngraph::Shape input_shape_padded = input_shape;
    ngraph::Shape padding_interior(data_dilation_strides);
    ...
    std::unique_ptr<nvidiagpu::primitive> kernel_launch(
        new nvidiagpu::primitive{[=](void** inputs, void** outputs) mutable {
            ...
            {
                nvidiagpu::invoke_primitive(m_ctx, conv_index, inputs, outputs);
            }
        }});
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::ConvolutionBackpropData* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
    auto input_shape = args[0].get_shape();
    ...
    ngraph::Shape padding_below_back(output_shape.size(), 0);
    ngraph::Shape padding_interior_back(output_shape.size(), 1);
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::ConvolutionBackpropFilters* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::MaxPool* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
    auto& input_shape = args[0].get_shape();
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::Max* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
    auto& input_shape = args[0].get_shape();
    ...
    else
    {
        ...
        kernel_launch.reset(new nvidiagpu::primitive{[=](void** inputs, void** outputs) mutable {
            nvidiagpu::invoke_primitive(m_ctx, max_index, inputs, outputs);
        }});
    }
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::Min* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
    auto& input_shape = args[0].get_shape();
    ...
    else
    {
        ...
        auto min_index = cudnn_emitter->build_reduce_forward(CUDNN_REDUCE_TENSOR_MIN,
                                                             ...
                                                             ReductionMode::Reduce);
        kernel_launch.reset(new nvidiagpu::primitive{[=](void** inputs, void** outputs) mutable {
            nvidiagpu::invoke_primitive(m_ctx, min_index, inputs, outputs);
        }});
    }
...
size_t runtime::nvidiagpu::CUDNNEmitter::build_primitive(const op::nvidiagpu::Rnn* node)
{
    auto& args = node->get_inputs();
    auto& out = node->get_outputs();
    auto dtype = out[0].get_element_type().c_type_string();
    ...
    ngraph::Shape hidden_shape{num_layers, batch_size, hidden_size};
    ...
    ngraph::Shape cell_state_shape{num_layers, batch_size, hidden_size};
    ...
    auto& temp_input_desc =
        get_nd_tensor_descriptor(ngraph::Shape{batch_size, input_size, 1}, data_type, format);

    size_t params_size = 0;
    CUDNN_SAFE_CALL(cudnnGetRNNParamsSize(
        *m_ctx->cudnn_handle, rnn_desc, temp_input_desc, &params_size, data_type));
    auto& w_desc = get_nd_filter_descriptor(ngraph::Shape{params_size, 1, 1}, data_type, format);
    ...
    std::unique_ptr<nvidiagpu::primitive> kernel_launch(
        new nvidiagpu::primitive{[=](void** inputs, void** outputs) {
            void* workspace_ptr = runtime::nvidiagpu::invoke_memory_primitive(m_ctx, workspace_idx);
                                                       ...
                                                       bool bprop_needs_pooling)
{
    ...
    case (Prop::Forward):
    {
        pool.reset(new nvidiagpu::primitive{
            [=, &desc, &input_desc, &output_desc](void** inputs, void** outputs) {
                                                         ...
                                                         bool invert_variance)
{
    ...
    void* alpha = m_host_parameters.allocate_by_datatype(data_type, 1.0);
    void* beta = m_host_parameters.allocate_by_datatype(data_type, 0);
    std::unique_ptr<nvidiagpu::primitive> batchnorm;
    switch (direction)
    {
    case Prop::Inference:
    {
        batchnorm.reset(new nvidiagpu::primitive{
    ...
    case Prop::Backward:
    {
        ...
        if (invert_variance)
        {
            ...
            auto reciprocal_idx = cuda_emitter->build_cudnn_bn_inv_var(
                {dtype, dtype}, ngraph::Shape{tensor_shape[1]}, epsilon);
            batchnorm.reset(new nvidiagpu::primitive{[=](void** inputs, void** outputs) {
                void* inv_var = runtime::nvidiagpu::invoke_memory_primitive(m_ctx, inv_var_idx);
                nvidiagpu::invoke_primitive(m_ctx, reciprocal_idx, &inputs[4], &inv_var);
                inputs[4] = inv_var;
                bnbp(inputs, outputs);
            }});
        }
                                                   ...
                                                   const size_t lrn_size)
{
    ...
    // emit lrn operation
    std::unique_ptr<nvidiagpu::primitive> lrn(new nvidiagpu::primitive{
...
void runtime::nvidiagpu::CUDNNEmitter::sync()
{
    CUDA_RT_SAFE_CALL(cudaDeviceSynchronize());
    return;
}

void runtime::nvidiagpu::CUDNNEmitter::debug_sync()
{
