                                                         ...
                                                         NodeMap& node_map)
{
    ...
    {
        auto result = node_map.at(node.get())->as_type_ptr<op::Result>();
        if (!result)
        {
    ...
    {
        cloned_params.push_back(node_map.at(param.get())->as_type_ptr<op::Parameter>());
    }
...
{
    if (auto rc = reduce_constant.get_node_shared_ptr()->as_type_ptr<ngraph::op::Constant>())
    {
