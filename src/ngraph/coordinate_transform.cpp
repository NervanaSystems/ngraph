    ...
    , m_target_dilation_strides(target_dilation_strides)
{
    ...
    // Fill out m_coordinates with all coordinates for this shape
    auto coordinate = Coordinate(m_target_shape.size(), 0);
    for (size_t i = 0; i < shape_size(m_target_shape); i++)
    {
        ...
        for (size_t axis = m_target_shape.size(); axis-- > 0;)
        {
            ...
            if (coordinate[axis] < m_target_shape[axis])
            {
                break;
            }
            else
