...
OutputVector builder::opset1::split(const Output<Node>& value,
                                    ...
                                    int64_t axis)
{
    const auto axis_node = op::v0::Constant::create(element::u64, Shape{}, {axis});
    const auto split_lengths_node =
        op::v0::Constant::create(element::u64, Shape{split_lengths.size()}, split_lengths);
    const auto variadic_split =
        std::make_shared<op::v1::VariadicSplit>(value, axis_node, split_lengths_node);
...
{
    const auto axis_node = op::v0::Constant::create(element::u64, Shape{}, {axis});
    const auto split = std::make_shared<op::v1::Split>(value, axis_node, num_splits);
