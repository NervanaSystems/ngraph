                                            ...
                                            std::string error_prefix)
{
    ...
    //
    Shape result_shape(image_dimension_count + 2);
                                        ...
                                        const std::shared_ptr<Node>& delta)
{
    ...
    adjoints.add_delta(
        x,
        std::make_shared<op::ConvolutionBackpropImageBatch>(x_shape,
                                                            f,
                                                            delta,
                                                            m_window_movement_strides,
                                                            m_window_dilation_strides,
    ...
    , m_image_dilation_strides_forward(image_dilation_strides_forward)
{
    ...
    {
        m_window_movement_strides_backward.push_back(image_dilation_strides_forward[i]);
        ...
        m_padding_above_backward.push_back(
            ...
            padding_above_forward[i]);
        m_image_dilation_strides_backward.push_back(window_movement_strides_forward[i]);
    }
    ...
    {
        throw ngraph_error(
            "Convolution image batch backprop inferred output shape does not match "
    ...
    , m_image_dilation_strides_forward(image_dilation_strides_forward)
{
    ...
    //
    // Make sure image batch and output delta element types match.
    //
    if (image_batch_et != output_delta_et)
    {
        throw ngraph_error(
            "Convolution filter backprop image batch and output delta element types do not match");
    }
    ...
    set_value_type_checked(image_batch_et, inferred_convolution_output_shape);
}
