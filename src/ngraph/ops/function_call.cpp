    ...
    , m_function(function)
{
    ...
    if (get_arguments_DEPRECATED().size() != function_params.size())
    {
        throw ngraph_error("Wrong number of arguments.");
    }

    for (size_t i = 0; i < get_arguments_DEPRECATED().size(); i++)
    {
        if (nullptr == get_arguments_DEPRECATED().at(i)->get_value_type())
        {
        ...
        if (*(get_arguments_DEPRECATED().at(i)->get_value_type()) !=
