        ...
        namespace op
        {
            namespace set_1
            {
                NodeVector onehot(const Node& node)
                {
                    ...
                    auto indices =
                        std::make_shared<ngraph::default_opset::Convert>(inputs.at(0), element::i64);
                    ...
                    std::shared_ptr<ngraph::Node> off_value = reshape::interpret_as_scalar(
                        std::make_shared<ngraph::opset0::Slice>(values, Coordinate{0}, Coordinate{1}));
                    std::shared_ptr<ngraph::Node> on_value = reshape::interpret_as_scalar(
                        std::make_shared<ngraph::opset0::Slice>(values, Coordinate{1}, Coordinate{2}));
