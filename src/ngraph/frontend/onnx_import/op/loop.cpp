        ...
        namespace op
        {
            namespace set_1
            {
                NodeVector loop(const Node& node)
                {
                    ...
                    // required
                    const Graph& body_graph{node.get_attribute_value<Graph>("graph")};
                    ...
                    for (const& dep : loop_carried_dependencies)
                    {
                        tensor_iterator->set_merged_input(
                            *graph_inputs_it++, dep, *graph_outputs_it);
                        final_values->get_iter_value(*graph_outputs_it, -1);
                        graph_outputs_it++;
                    }
                    ...
                    NodeVector node_outputs(ngraph::as_node_vector(final_values));
                    for (const& value : scan_outputs)
                    {
                        node_outputs.push_back(value.as_single_output_node());
                    }
                    return node_outputs;
                }
            } // namespace set_1
