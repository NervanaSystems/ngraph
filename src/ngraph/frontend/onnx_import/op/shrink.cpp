            ...
            namespace set_1
            {
                NodeVector shrink(const Node& node)
                {
                    ...
                    ASSERT_VALID_ARGUMENT(node, !(lambd < 0.0f))
                            << " The provided 'lambd' value:" << lambd
                            << " must not be negative.";

                    const auto negative_lambd = ngraph::op::Constant::create(input->get_element_type(),
                            input->get_shape(), {-lambd});

                    const auto positive_lambd = ngraph::op::Constant::create(input->get_element_type(),
                            input->get_shape(), {lambd});

                    const auto bias_tensor = ngraph::op::Constant::create(input->get_element_type(),
                            input->get_shape(), {bias});
                    ...
                    std::shared_ptr<ngraph::Node> values_below_neg_lambd = std::make_shared<ngraph::op::Less>(input, negative_lambd);
                    std::shared_ptr<ngraph::Node> values_above_pos_lambd = std::make_shared<ngraph::op::Greater>(input, positive_lambd);
