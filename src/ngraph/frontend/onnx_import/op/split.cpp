...
static ngraph::NodeVector get_outputs(const std::shared_ptr<ngraph::Node>& node, int64_t outputs_numer)
{
        ...
        namespace op
        {
            namespace set_1
            {
                NodeVector split(const Node& node)
                {
                    ...
                    const auto axis_node = ngraph::op::Constant::create(element::i64, Shape{}, {axis});
                    ...
                    try
                    {
                        ...
                        fused_split = std::make_shared<ngraph::op::Split>(input, axis_node, length_parts);
                    }
                    catch (const error::node::UnknownAttribute&)
                    {
                        ...
                        fused_split = std::make_shared<ngraph::op::Split>(input, axis_node, outputs_number);
                    }
