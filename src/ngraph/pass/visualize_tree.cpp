...
std::string pass::VisualizeTree::get_attributes(shared_ptr<Node> node)
{
    ...
    {
        //The shapes of the Outputs of a multi-output op will be printed for its corresponding `GetOutputElement`s
        ss << " " << (node->get_outputs().size() != 1 ? std::string("[skipped]") : vector_to_string(node->get_shape()));
    }
