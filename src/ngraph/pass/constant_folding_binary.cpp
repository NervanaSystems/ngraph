                                              ...
                                              NodeExecutorTy func)
{
    ...
    {
        if (auto add_node = binary->as_type_ptr<op::Add>())
        {
        ...
        else if (auto and_node = binary->as_type_ptr<op::And>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto divide_node = binary->as_type_ptr<op::Divide>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
            shared_ptr<op::Divide> divop = binary->as_type_ptr<op::Divide>();
        ...
        else if (auto equal_node = binary->as_type_ptr<op::Equal>())
        {
        ...
        else if (auto greater_node = binary->as_type_ptr<op::Greater>())
        {
            NGRAPH_CHECK(element::from<Tout>() == element::boolean, "Output type is not boolean");
            vector<char> out_vec(shape_size(out_shape));
        ...
        else if (auto greater_eq_node = binary->as_type_ptr<op::GreaterEq>())
        {
            NGRAPH_CHECK(element::from<Tout>() == element::boolean, "Output type is not boolean");
            vector<char> out_vec(shape_size(out_shape));
        ...
        else if (auto less_node = binary->as_type_ptr<op::Less>())
        {
            NGRAPH_CHECK(element::from<Tout>() == element::boolean, "Output type is not boolean");
            vector<char> out_vec(shape_size(out_shape));
        ...
        else if (auto less_eq_node = binary->as_type_ptr<op::LessEq>())
        {
            NGRAPH_CHECK(element::from<Tout>() == element::boolean, "Output type is not boolean");
            vector<char> out_vec(shape_size(out_shape));
        ...
        else if (auto maximum_node = binary->as_type_ptr<op::Maximum>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto minimum_node = binary->as_type_ptr<op::Minimum>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto multiply_node = binary->as_type_ptr<op::Multiply>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto not_equal_node = binary->as_type_ptr<op::NotEqual>())
        {
            NGRAPH_CHECK(element::from<Tout>() == element::boolean, "Output type is not boolean");
            vector<char> out_vec(shape_size(out_shape));
        ...
        else if (auto or_node = binary->as_type_ptr<op::Or>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto subtract_node = binary->as_type_ptr<op::Subtract>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
        ...
        else if (auto xor_node = binary->as_type_ptr<op::Xor>())
        {
            NGRAPH_CHECK(element::from<Tin>() == element::from<Tout>(),
                         "Input/output types do not match");
            vector<Tin> out_vec(shape_size(out_shape));
...
void pass::ConstantFolding::construct_constant_binary()
{
    ...
    auto constant_binary_callback = [&, a, b](pattern::Matcher& m) {
        ...
        auto a_match = pattern_map[a]->as_type_ptr<op::Constant>();
        auto b_match = pattern_map[b]->as_type_ptr<op::Constant>();
