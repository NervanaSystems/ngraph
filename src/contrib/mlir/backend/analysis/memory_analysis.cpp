...
public:
  MemoryAssignment(MemoryAnalysis *memAnalysis) : m_memAnalysis(memAnalysis) {
    m_inplaceOps = {
...
void MemoryAssignment::processConcat(mlir::Operation *op) {
  auto concat = cast<mlir::NGConcatOp>(op);
  {
    ...
    for (auto opnd : op->getOperands()) {
      if (!m_aliasRelation.canAlias(result, opnd) ||
...
void MemoryAssignment::processDestructiveInPlace(mlir::Operation *op) {
  NGRAPH_CHECK(op->getNumResults() == 1,
               "Destructive in-place with multi-def ?");
  Value use = nullptr;
  int useCount = -1;
