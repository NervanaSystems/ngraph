...
=======
        // Initialize output to zero
        {
            ...
            auto resSpatialIndicesPtrs =
                makeHandlePointers(MutableArrayRef<IndexHandle>(resSpatialIndices));
>>>>>>> 1c43db90d20e5e6685e5136a5b20399aedf14bac
                ...
                AffineLoopNestBuilder(
                    ...
                    AffineLoopNestBuilder(filtersSpatialIndicesPtrs,
                        ...
                        if (withPadding)
                        {
                            ...
                            auto affineIfOp = rewriter.create<AffineIfOp>(rewriter.getUnknownLoc(),
                                                                          nonPaddedRange,
                                                                          affineIfArgs,
                                                                          /*withElseRegion=*/false);
                            {
                                ...
                                iRes(resIndices) = iRes(resIndices) + (iImages(adjustedImgIndices) *
                         ...
                         DialectLoweringPass& pass)
{
    ...
    // Generate loop nest that computes the actual index reduction.
    {
        ...
        Type resTy = result.getType().cast<MemRefType>().getElementType();
        NGRAPH_CHECK(resTy.isa<IntegerType>(), "Expected integer result type in index reduction");
                  ...
                  DialectLoweringPass& pass)
{
    ...
    FuncOp callBackFunc = pass.getCallDecl("__mlir_callback_1_input",
                                           {unrankedMemrefTy, unrankedMemrefTy, int64Ty, int64Ty},
                                           {},
                                           rewriter);
    ...
    auto index = insertAttrs(attrs);
    auto attrsIndexArg = rewriter.create<mlir::ConstantIntOp>(rewriter.getUnknownLoc(), index, 64);
    auto opTypeArg = rewriter.create<mlir::ConstantIntOp>(
