  ...
  if (!parser.parseOptionalKeyword("tensor")) {
    ...
    if (parser.parseLess()) {
      parser.emitError(typeLoc,
                       "expected '<' and '>' enclosing the tensor shape");
      return Type();
    }
...
mlir::Type
NGraphOpsDialect::parseEltType(mlir::DialectAsmParser &parser) const {
  ...
  if (tyData.startswith("i") || tyData.startswith("u")) {
    ...
    // successfully.
    if (tyData.consumeInteger(/*Radix=*/10, width) || width == 0 ||
        !tyData.empty()) {
    ...
    if (!(width == 8 || width == 16 || width == 32 || width == 64)) {
      parser.emitError(loc, "Unexpected width = " + std::to_string(width) +
...
void NGraphOpsDialect::printType(mlir::Type type,
