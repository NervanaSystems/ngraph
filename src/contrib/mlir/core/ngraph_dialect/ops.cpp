...
verifyCompatibleOperandsAndResults(T op, bool checkResult = true) {
...
template <typename T> static mlir::LogicalResult verifyUnaryArithOp(T op) {
  return verifyCompatibleOperandsAndResults(op);
}

template <typename T> static mlir::LogicalResult verifyBinaryArithOp(T op) {
  return verifyCompatibleOperandsAndResults(op);
}

template <typename T> static mlir::LogicalResult verifyAxisReductionOp(T op) {
  return mlir::failure();
}
...
template <typename T> static mlir::LogicalResult verifyIndexReductionOp(T op) {
  // TODO: verifyAxisReductionOp(op) + return element type + single axis.
  return mlir::success();
}

template <typename T> static mlir::LogicalResult verifyOp(T op) {
  return op.emitOpError("Unsupported verifier for this operation");
}

template <> mlir::LogicalResult verifyOp(NGDotOp op) {
  // TODO(dcab): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGConcatOp op) {
  // TODO(amprocte): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGSelectOp op) {
  mlir::Type t0 = op.getOperation()->getOperand(0).getType();
  ...
  if (!opType0.isCompatibleShape(opType1) ||
      !opType0.getElementType().isa<NGBoolType>())
...
template <typename T> static mlir::LogicalResult verifyCmpOp(T op) {
  mlir::LogicalResult result =
      verifyCompatibleOperandsAndResults(op, false /*checkResult*/);
  if (failed(result)) {
    return result;
  }
  ...
  if (!resType.isCompatibleShape(opType0) ||
      !resType.getElementType().cast<mlir::IntegerType>().isUnsignedInteger(
          8)) {
    return op.emitOpError(
        "Incompatible result shape or type for comparison op");
  }

  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGGatherOp op) {
  ...
  if (!indicesEltType.isSignedInteger(32) &&
      !indicesEltType.isSignedInteger(64))
...
template <> mlir::LogicalResult verifyOp(NGConvolutionOp op) {
  ...
  if (imageSpatialRank != stridesRank || imageSpatialRank != padBelowRank ||
    ...
    return op.emitOpError(
        "Image spatial rank mismatches strides and/or padding ranks");
  }
  ...
  if (imagesShape[0] <= 0 || filtersShape[0] <= 0 ||
      imagesShape[1] != filtersShape[1] || imagesShape[1] <= 0) {
  ...
  for (unsigned i = 0; i < resultRank - 2; i++) {
    unsigned resDim =
        llvm::divideCeil(padBelowVal[i] + padAboveVal[i] + imagesShape[2 + i] -
                             filtersShape[2 + i] + 1,
                         stridesVal[i]);
...
template <> mlir::LogicalResult verifyOp(NGSoftMaxOp op) {
  // TODO(ayzhuang): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGAvgPoolOp op) {
  // TODO(ayzhuang): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGAvgPoolBackpropOp op) {
  // TODO(ayzhuang): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGMaxPoolOp op) {
  // TODO(ayzhuang): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGMaxPoolBackpropOp op) {
  // TODO(ayzhuang): Improve verification: proper shapes, etc.
  return mlir::success();
}

template <> mlir::LogicalResult verifyOp(NGSliceOp op) {
  NGTensorType t0 =
      op.getOperation()->getOperand(0).getType().cast<NGTensorType>();
  NGTensorType r0 =
      op.getOperation()->getResult(0).getType().cast<NGTensorType>();
  ...
  for (auto i = 0; i < t0.getRank(); i++) {
    ...
    if (resShape[i] > srcShape[i]) {
      return op.emitOpError()
             << "Output shape at " << i << " dimension cannot exceed"
             << "input shape dimensions";
    }
    if (lb > ub) {
      return op.emitOpError()
             << "Lower bound at " << i << " dimension is above upper bound";
    }
    if (lb < 0) {
      return op.emitOpError()
             << "Lower bound at " << i << " dimension must be non-negative";
    }
    if (ub > srcShape[i]) {
      return op.emitOpError()
             << "Upper bound at " << i << " dimension is out of range";
    }
    if (st < 0) {
