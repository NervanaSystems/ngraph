...
namespace
{
    ...
    /// Container that represents a set of ops to be fused together. Each fusion group has a
    /// predominant kind that determines which ops can be fused into this group.
    ...
    /// Op-based fusion pass that makes fusion decisions by taking advantage of nGraph ops' high
    /// level information. The algorithm classifies nGraph ops into multiple categories and fuses
    /// ops by using generic heuristics based on those categories. Currently, it only prints the
    /// fused ops into the dbgs output.
    ///
    /// Current TODOs:
    ...
    private:
        /// Create a new fusion group with 'op' as the first member of the group. The group kind is
        /// initialize to the op's fusion kind.
...
/// This utility implements the logic to fuse 'op' into the fusion group 'currGroup'.
...
/// Create a new fusion group with 'op' as the first member of the group. The group kind is
/// initialize to the 'op' fusion kind.
