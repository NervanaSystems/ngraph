...
#define COMPILE_OP_DECL(op_name)                                               \
  createOp<op_name>(NgDialectConversionPass & NgDialectObj,                    \
...
class NgDialectConversionPass
    : public mlir::PassWrapper<NgDialectConversionPass,
...
private:
  ...
  template <typename T> mlir::ArrayAttr getShapeAsAttr(T ngShape);
  /// Returns the builder
...
private:
  ...
  using MLIRCompOpMap =
      std::unordered_map<Node::type_info_t, MLIRCompOpFunction>;
...
NgDialectConversionPass::NgDialectConversionPass(
    ...
    : m_compiledKernel(obj.m_compiledKernel), m_context(obj.m_context),
      m_builder(obj.m_builder), m_tensorToValueMap(obj.m_tensorToValueMap) {}
...
void NgDialectConversionPass::runOnOperation() {
  ...
  auto funcType =
      mlir::FunctionType::get(argsTypeList, resultTypeList, m_context);
  auto function =
...
template <typename T>
void NgDialectConversionPass::getMlirShape(
    T ngShape, llvm::SmallVectorImpl<int64_t> &mlirShape) {
...
// element type.
mlir::Type
NgDialectConversionPass::getMlirType(const descriptor::Tensor *tensor) {
...
#pragma GCC diagnostic error "-Wswitch-enum"
  ...
  default:
    NGRAPH_CHECK(false, "MLIR: Unsupported NGraph types");
    break;
  case ngraph::element::Type_t::bf16:
    return mlir::NGFloatType::getBF16(m_context);
  case ngraph::element::Type_t::f16:
    return mlir::NGFloatType::getF16(m_context);
  case ngraph::element::Type_t::f32:
    return mlir::NGFloatType::getF32(m_context);
  case ngraph::element::Type_t::f64:
    return mlir::NGFloatType::getF64(m_context);
  case ngraph::element::Type_t::i8:
    return mlir::NGIntegerType::get(8, sign, m_context);
  case ngraph::element::Type_t::u8:
  case ngraph::element::Type_t::boolean:
    return mlir::NGIntegerType::get(8, unsign, m_context);
  case ngraph::element::Type_t::i16:
    return mlir::NGIntegerType::get(16, sign, m_context);
  case ngraph::element::Type_t::u16:
    return mlir::NGIntegerType::get(16, unsign, m_context);
  case ngraph::element::Type_t::i32:
    return mlir::NGIntegerType::get(32, sign, m_context);
  case ngraph::element::Type_t::u32:
    return mlir::NGIntegerType::get(32, unsign, m_context);
  case ngraph::element::Type_t::i64:
    return mlir::NGIntegerType::get(64, sign, m_context);
  case ngraph::element::Type_t::u64:
    return mlir::NGIntegerType::get(64, unsign, m_context);
  }
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Subtract) {
  return NgDialectObj.createGenericOp<mlir::NGSubOp>(ngNode);
}

template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Multiply) {
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::GreaterEq) {
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::NotEqual) {
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Concat) {
  ...
  op->setAttr("concatenation_axis", NgDialectObj.m_builder.getI64IntegerAttr(
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Gather) {
  ...
  op->setAttr("axis",
              NgDialectObj.m_builder.getI64IntegerAttr(gather->get_axis()));
  return op;
}
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Negative) {
  return NgDialectObj.createGenericOp<mlir::NGNegOp>(ngNode);
}

template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Convolution) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGConvolutionOp>(ngNode);
  auto convNode = static_cast<const ngraph::op::Convolution *>(ngNode);
  auto convOp = llvm::cast<mlir::NGConvolutionOp>(op);

  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(convNode->get_window_movement_strides());
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::GroupConvolution) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGGroupConvOp>(ngNode);
  auto gConvNode = static_cast<const ngraph::op::GroupConvolution *>(ngNode);
  auto gConvOp = llvm::cast<mlir::NGGroupConvOp>(op);

  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(gConvNode->get_window_movement_strides());
  ...
  gConvOp.setGroups(
      NgDialectObj.getBuilder().getI64IntegerAttr(gConvNode->get_groups()));
  return op;
}

template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::ConvolutionBias) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGConvBiasOp>(ngNode);
  ...
  convOp.setStrides(
      NgDialectObj.getShapeAsAttr(convNode->get_window_movement_strides()));
  convOp.setDilation(
      NgDialectObj.getShapeAsAttr(convNode->get_window_dilation_strides()));
  convOp.setPadBelow(
      NgDialectObj.getShapeAsAttr(convNode->get_padding_below()));
  convOp.setPadAbove(
      NgDialectObj.getShapeAsAttr(convNode->get_padding_above()));
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::AvgPool) {
  ...
  mlir::BoolAttr boolAttr = NgDialectObj.m_builder.getBoolAttr(
      avgPoolNode->get_include_padding_in_avg_computation());
  avgPoolOp.setIncludePadding(boolAttr);

  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(avgPoolNode->get_window_shape());
  avgPoolOp.setWindowShape(attr);

  attr =
      NgDialectObj.getShapeAsAttr(avgPoolNode->get_window_movement_strides());
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::AvgPoolBackprop) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGAvgPoolBackpropOp>(ngNode);
  auto avgPoolBackpropNode =
      static_cast<const ngraph::op::AvgPoolBackprop *>(ngNode);
  ...
  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(avgPoolBackpropNode->get_window_shape());
  avgPoolBackpropOp.setWindowShape(attr);

  attr = NgDialectObj.getShapeAsAttr(
      avgPoolBackpropNode->get_window_movement_strides());
  ...
  attr =
      NgDialectObj.getShapeAsAttr(avgPoolBackpropNode->get_forward_arg_shape());
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::MaxPool) {
  ...
  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(maxPoolNode->get_window_shape());
  maxPoolOp.setWindowShape(attr);

  attr =
      NgDialectObj.getShapeAsAttr(maxPoolNode->get_window_movement_strides());
...
template <>
mlir::Operation *
NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::MaxPoolBackprop) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGMaxPoolBackpropOp>(ngNode, 2);
  ...
  mlir::ArrayAttr attr =
      NgDialectObj.getShapeAsAttr(maxPoolBackpropNode->get_window_shape());
  maxPoolBackpropOp.setWindowShape(attr);

  attr = NgDialectObj.getShapeAsAttr(
      maxPoolBackpropNode->get_window_movement_strides());
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::MatMul) {
  ...
  matmulOp.setTransposeA(
      NgDialectObj.m_builder.getBoolAttr(matmulNode->get_transpose_a()));
  matmulOp.setTransposeB(
      NgDialectObj.m_builder.getBoolAttr(matmulNode->get_transpose_b()));
  return op;
}
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Gemm) {
  ...
  gemmOp.setAlpha(
      NgDialectObj.m_builder.getF32FloatAttr(gemmNode->get_alpha()));
...
mlir::Operation *NgDialectConversionPass::COMPILE_OP_DECL(ngraph::op::Softmax) {
  mlir::Operation *op =
      NgDialectObj.createGenericOp<mlir::NGSoftMaxOp>(ngNode, 1);
...
template <typename Op>
mlir::Operation *
NgDialectConversionPass::createGenericOp(const ngraph::Node *ngNode,
                                         int inNum) {
  ...
  return (m_builder.create<Op, ArrayRef<mlir::Type>, ArrayRef<mlir::Value>,
                           ArrayRef<mlir::NamedAttribute>>(
              mlir::UnknownLoc::get(m_context), resTypes, argValues,
              {/* no attrs */}))
      .getOperation();
}

const NgDialectConversionPass::MLIRCompOpMap &
NgDialectConversionPass::getOpDispatcher() {
  static MLIRCompOpMap opDispatcher{
#define MLIR_OP(OP)                                                            \
  {ngraph::op::OP::type_info,                                                  \
   &NgDialectConversionPass::createOp<ngraph::op::OP>},
...
void NgDialectConversionPass::createReturn() {
  ...
  m_builder.create<mlir::NGReturnOp>(mlir::UnknownLoc::get(m_context),
                                     valueList);
}

template <typename RedOp>
mlir::Operation *
NgDialectConversionPass::createIndexReduction(const ngraph::Node *ngNode) {
...
std::unique_ptr<mlir::Pass> ngraph::pass::createNgDialectConversionPass(
    const ngraph::op::CompiledKernel *compiledKernel,
    mlir::MLIRContext *context) {
