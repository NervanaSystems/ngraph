...
void MLIRSubgraphExtractionPass::MLIRSubgraph::add_node(
  ...
  NGRAPH_CHECK(m_pass.m_node_to_graph.find(node) ==
                   m_pass.m_node_to_graph.end(),
...
bool MLIRSubgraphExtractionPass::run_on_function(
...
static void process_successors(
...
void MLIRSubgraphExtractionPass::process_supported_op(
    std::shared_ptr<ngraph::Node> node, int current_subgraph_id) {
  NodeVector inputs;
...
void MLIRSubgraphExtractionPass::build_subgraphs(
    std::shared_ptr<Function> func) {
  ...
  for (IDGraphMap::iterator it = m_id_to_graph.begin();
...
MLIRSubgraphExtractionPass::build_ck_nodes(std::shared_ptr<Function> func) {
  ...
  for (auto it : m_id_to_graph) {
    ...
    auto ck = std::make_shared<CompiledKernel>(nodes_vector, outputs_vector,
                                               inputs_vector);
  ...
  for (auto &node : ck_nodes) {
    ...
    std::unordered_set<std::shared_ptr<Node>> node_set(node_list.begin(),
                                                       node_list.end());
    ...
    for (size_t i = 0, end = outputs_vector.size(); i < end; ++i) {
      ...
      for (auto input : inputs) {
        if (node_set.find(input.get_node()->shared_from_this()) ==
            node_set.end()) {
...
void MLIRSubgraphExtractionPass::sanity_check(std::shared_ptr<Function> func,
                                              NodeVector &ck_nodes) {
  NodeVector cycles;
  bool is_bkwd_cycle;
  ...
  for (auto &node : ck_nodes) {
    ...
    for (auto &ck_output : ck_node->get_kernel_outputs()) {
      for (auto &user : ck_output.get_target_inputs()) {
        NGRAPH_CHECK(node_set.find(user.get_node()->shared_from_this()) !=
                         node_set.end(),
...
static bool can_use_mkldnn_conv_callback(ngraph::Node *node) {
  ...
  if (std::any_of(pad_below.begin(), pad_below.end(),
                  [](size_t s) { return s < 0; })) {
    return false;
  }
...
bool MLIRSubgraphExtractionPass::is_supported_mlir_op(
    std::shared_ptr<Node> node) {
  ...
  if (is_type<ngraph::op::Dot>(node)) {
    if (node->get_input_shape(0).size() != 2 ||
  ...
  if (auto avg_pool_backprop = as_type_ptr<ngraph::op::AvgPoolBackprop>(node)) {
    ...
    return ((arg0_rank == 4 &&
             avg_pool_backprop->get_window_shape().size() == 2) ||
            (arg0_rank == 5 &&
             avg_pool_backprop->get_window_shape().size() == 3)) &&
           node->get_input_element_type(0) == element::f32;
  }
  ...
  if (auto max_pool_backprop = as_type_ptr<ngraph::op::MaxPoolBackprop>(node)) {
    ...
    return ((arg0_rank == 4 &&
             max_pool_backprop->get_window_shape().size() == 2) ||
            (arg0_rank == 5 &&
             max_pool_backprop->get_window_shape().size() == 3)) &&
           node->get_input_element_type(0) == element::f32;
  }
...
const std::set<ngraph::Node::type_info_t> &
MLIRSubgraphExtractionPass::getSupportedOps() {
