...
static bool inline compareMkldnnDims(dnnl_dims_t &arr1, dnnl_dims_t &arr2,
                                     size_t size) {
...
static bool compareMkldnnStridesOrder(dnnl_dims_t &strides1,
                                      dnnl_dims_t &strides2, size_t size) {
...
static bool compareMkldnnMdFormats(const dnnl::memory::desc &lhs,
  ...
  if (md1.format_kind !=
      static_cast<dnnl_format_kind_t>(dnnl::memory::format_kind::blocked)) {
...
static void convertOutputLayout(dnnl::memory &reorderIn,
                                const dnnl::memory::desc &rhs, void *ptr,
...
static void __mlir_dnnl_convbias(size_t rank, StaticMemRef *memRefData,
  ...
  auto dataDesc =
      dnnl::memory::desc(dataDims, dtype, dnnl::memory::FORMAT::any);
  auto dataDescOrigin = dnnl::memory::desc(dataDims, dtype, dataStrides);
  auto weightsDesc =
      dnnl::memory::desc(weightsDims, dtype, dnnl::memory::FORMAT::any);
  auto weightsDescOrigin =
      dnnl::memory::desc(weightsDims, dtype, weightsStrides);
  auto biasDesc =
  ...
  ops.append_eltwise(opsScale, dnnl::algorithm::eltwise_relu, opsAlpha,
                     opsBeta);
  if (rank == 3) {
...
static void __mlir_dnnl_maxpoolbackprop(size_t rank, StaticMemRef *memRefSrc,
  ...
  // build dnnl primitive and execute
  auto requiredFormat =
      rank == 4 ? dnnl::memory::FORMAT::nchw : dnnl::memory::FORMAT::ncdhw;
  dnnl::memory::data_type dtype = dnnl::memory::data_type::f32;
  ...
  std::unordered_map<int, dnnl::memory> execArgsF = {
      ...
      {DNNL_ARG_DST, dstMem}};
  std::unordered_map<int, dnnl::memory> execArgsB = {
      {DNNL_ARG_DIFF_DST, diffDst},
      {DNNL_ARG_WORKSPACE, workspace},
...
static void __mlir_dnnl_softmax(size_t rank, StaticMemRef *memRefInput,
  ...
  dnnl::engine cpuEngine(dnnl::engine::kind::cpu, 0);
  try {
    auto softmaxDesc = dnnl::softmax_forward::desc(
        dnnl::prop_kind::forward_scoring, inputDesc, softmaxAxis);
    dnnl::primitive_attr attr;
    softmaxPd =
        dnnl::softmax_forward::primitive_desc(softmaxDesc, attr, cpuEngine);
  } catch (const dnnl::error &e) {
    throw ngraph_error("Could not create dnnl softmax descriptor " +
                       std::string(e.message));
  }
