...
static bool inline compareMkldnnDims(mkldnn_dims_t &arr1, mkldnn_dims_t &arr2,
                                     size_t size) {
...
static bool compareMkldnnStridesOrder(mkldnn_dims_t &strides1,
                                      mkldnn_dims_t &strides2, size_t size) {
  std::vector<size_t> indices1(size, 0), indices2(size, 0);
  for (size_t i = 0; i < size; i++) {
    indices1[i] = i;
    indices2[i] = i;
  }
  std::sort(indices1.begin(), indices1.begin(),
            [&](const size_t &n1, const size_t &n2) {
              return strides1[n1] < strides1[n2];
            });
  std::sort(indices2.begin(), indices2.begin(),
            [&](const size_t &n1, const size_t &n2) {
...
static bool compareMkldnnMdFormats(const mkldnn::memory::desc &lhs,
  ...
  if (md1.format_kind !=
      static_cast<mkldnn_format_kind_t>(mkldnn::memory::format_kind::blocked)) {
...
static mkldnn::memory convertLayoutIfDiff(const mkldnn::memory::desc &lhs,
                                          const mkldnn::memory::desc &rhs,
                                          void *ptr, mkldnn::engine cpuEngine) {
  if (!compareMkldnnMdFormats(lhs, rhs)) {
    ...
    std::unordered_map<int, mkldnn::memory> execArgs = {
        {MKLDNN_ARG_SRC, reorderIn}, {MKLDNN_ARG_DST, reorderOut}};
    mkldnn::stream s(cpuEngine);
    try {
      convert.execute(s, execArgs);
      s.wait();
    } catch (const mkldnn::error &e) {
      throw ngraph_error("Could not run mkdnn primitive " +
...
static void convertOutputLayout(mkldnn::memory &reorderIn,
                                const mkldnn::memory::desc &rhs, void *ptr,
...
static void __mlir_mkldnn_convbias(size_t rank, StaticMemRef *memRefData,
  ...
  auto dataDesc =
      mkldnn::memory::desc(dataDims, dtype, mkldnn::memory::FORMAT::any);
  auto dataDescOrigin = mkldnn::memory::desc(dataDims, dtype, dataStrides);
  auto weightsDesc =
      mkldnn::memory::desc(weightsDims, dtype, mkldnn::memory::FORMAT::any);
  auto weightsDescOrigin =
      mkldnn::memory::desc(weightsDims, dtype, weightsStrides);
  auto biasDesc =
      mkldnn::memory::desc(biasDims, dtype, mkldnn::memory::FORMAT::any);
  auto resultDesc =
      mkldnn::memory::desc(resultDims, dtype, mkldnn::memory::FORMAT::any);
  auto resultDescOrigin =
      mkldnn::memory::desc(resultDims, dtype, resultStrides);
  ...
  ops.append_eltwise(opsScale, mkldnn::algorithm::eltwise_relu, opsAlpha,
                     opsBeta);
  if (rank == 3) {
    auto convAttrs = (*attrsPtr).convAttrs1d;
    try {
      auto convDesc = mkldnn::convolution_forward::desc(
          mkldnn::prop_kind::forward_inference, alg, dataDesc, weightsDesc,
          biasDesc, resultDesc,
      ...
      convPd = mkldnn::convolution_forward::primitive_desc(convDesc, attr,
                                                           cpuEngine);
    } catch (const mkldnn::error &e) {
      throw ngraph_error("Could not create mkldnn conv descriptor " +
                         std::string(e.message));
    }
  } else if (rank == 4) {
  ...
  if (needConvert) {
    convertOutputLayout(out, resultDescOrigin, memRefOutput->allocatedPtr,
                        cpuEngine);
  }
}

/// Callback for MaxPoolBackprop
static void __mlir_mkldnn_maxpoolbackprop(size_t rank, StaticMemRef *memRefSrc,
  ...
  // build mkldnn primitive and execute
  auto requiredFormat =
      rank == 4 ? mkldnn::memory::FORMAT::nchw : mkldnn::memory::FORMAT::ncdhw;
  mkldnn::memory::data_type dtype = mkldnn::memory::data_type::f32;
  ...
  if (rank == 4) {
    ...
    try {
      auto maxpoolDescF = mkldnn::pooling_forward::desc(
          ...
          mkldnn::memory::dims{pAttrs.windowStrides[0],
      ...
      auto maxpoolDescB = mkldnn::pooling_backward::desc(
          mkldnn::algorithm::pooling_max, diffSrcDesc, diffDstDesc,
          mkldnn::memory::dims{pAttrs.windowStrides[0],
                               pAttrs.windowStrides[1]},
          mkldnn::memory::dims{pAttrs.windowShape[0], pAttrs.windowShape[1]},
          mkldnn::memory::dims{pAttrs.padBelow[0], pAttrs.padBelow[1]},
          mkldnn::memory::dims{pAttrs.padAbove[0], pAttrs.padAbove[1]});
      maxpoolPdF = mkldnn::pooling_forward::primitive_desc(maxpoolDescF, attr,
                                                           cpuEngine);
      maxpoolPdB = mkldnn::pooling_backward::primitive_desc(
  ...
  mkldnn::memory srcMem =
      convertLayoutIfDiff(srcDescOrigin, maxpoolPdB.diff_src_desc(),
                          memRefSrc->allocatedPtr, cpuEngine);
  ...
  mkldnn::memory diffDst =
      convertLayoutIfDiff(diffDstDescOrigin, maxpoolPdB.diff_dst_desc(),
                          memRefDelta->allocatedPtr, cpuEngine);
  ...
  std::unordered_map<int, mkldnn::memory> execArgsF = {
      {MKLDNN_ARG_SRC, srcMem},
      {MKLDNN_ARG_WORKSPACE, workspace},
      {MKLDNN_ARG_DST, dstMem}};
  std::unordered_map<int, mkldnn::memory> execArgsB = {
      {MKLDNN_ARG_DIFF_DST, diffDst},
      {MKLDNN_ARG_WORKSPACE, workspace},
  ...
  if (needConvert) {
    convertOutputLayout(diffSrc, diffSrcDescOrigin, memRefOutput->allocatedPtr,
                        cpuEngine);
  }
}
...
static void __mlir_mkldnn_avgpoolbackprop(size_t rank,
  ...
  if (rank == 4) {
    ...
    try {
      ...
      auto avgpoolDescB = mkldnn::pooling_backward::desc(
          ...
          mkldnn::memory::dims{pAttrs.padBelow[0], pAttrs.padBelow[1]},
          mkldnn::memory::dims{pAttrs.padAbove[0], pAttrs.padAbove[1]});
      auto avgpoolPdF = mkldnn::pooling_forward::primitive_desc(
          avgpoolDescF, attr, cpuEngine);
      avgpoolPdB = mkldnn::pooling_backward::primitive_desc(
  ...
  mkldnn::memory in =
      convertLayoutIfDiff(diffDstDescOrigin, avgpoolPdB.diff_dst_desc(),
  ...
  std::unordered_map<int, mkldnn::memory> execArgs = {
      {MKLDNN_ARG_DIFF_DST, in}, {MKLDNN_ARG_DIFF_SRC, out}};
  ...
  if (needConvert) {
    convertOutputLayout(out, diffSrcDescOrigin, memRefOutput->allocatedPtr,
                        cpuEngine);
  }
}

/// Callback for AvgPool and MaxPool
static void __mlir_mkldnn_pooling(size_t rank, StaticMemRef *memRefInput,
  ...
  if (rank == 4) {
    ...
    try {
      auto poolDesc = mkldnn::pooling_forward::desc(
          mkldnn::prop_kind::forward_inference, alg, inputDesc, resultDesc,
          mkldnn::memory::dims{pAttrs.windowStrides[0],
                               pAttrs.windowStrides[1]},
          mkldnn::memory::dims{pAttrs.windowShape[0], pAttrs.windowShape[1]},
          mkldnn::memory::dims{pAttrs.padBelow[0], pAttrs.padBelow[1]},
          mkldnn::memory::dims{pAttrs.padAbove[0], pAttrs.padAbove[1]});
      poolPd =
          mkldnn::pooling_forward::primitive_desc(poolDesc, attr, cpuEngine);
    } catch (const mkldnn::error &e) {
  ...
  mkldnn::memory in = convertLayoutIfDiff(inputDescOrigin, poolPd.src_desc(),
                                          memRefInput->allocatedPtr, cpuEngine);
  mkldnn::memory out;
  bool needConvert = false;
...
static void __mlir_mkldnn_softmax(size_t rank, StaticMemRef *memRefInput,
                                  StaticMemRef *memRefOutput,
                                  opAttrs *attrsPtr) {
  mkldnn::memory::dims dims(rank);
  mkldnn::memory::dims strides(rank);
  ...
  try {
    ...
    mkldnn::primitive_attr attr;
    softmaxPd =
        mkldnn::softmax_forward::primitive_desc(softmaxDesc, attr, cpuEngine);
  } catch (const mkldnn::error &e) {
    throw ngraph_error("Could not create mkldnn softmax descriptor " +
                       std::string(e.message));
  }
  mkldnn::softmax_forward softmax(softmaxPd);

  mkldnn::memory in{softmaxPd.src_desc(), cpuEngine, memRefInput->allocatedPtr};
  mkldnn::memory out{softmaxPd.dst_desc(), cpuEngine,
                     memRefOutput->allocatedPtr};
...
static void __mlir_cblas_sgemm(StaticMemRef *memRefmatA,
                               ...
                               StaticMemRef *memRefmatC, opAttrs *attrsPtr) {
...
static void __mlir_cblas_sgemm_with_bias(StaticMemRef *memRefmatA,
  ...
  cblas::cblas_sgemm(
      ...
      transposeB ? cblas::Transpose::Transpose : cblas::Transpose::None, m, n,
      k, alpha, matA, std::max<size_t>(1, lda), matB, std::max<size_t>(1, ldb),
      0.0f, matOut, std::max<size_t>(1, ldc));

  if (broadcastHint == BroadcastType::ROW) {
    std::vector<float> ones(m, 1.0f);
    cblas::cblas_sgemm(cblas::Layout::RowMajor, cblas::Transpose::None,
                       cblas::Transpose::None, m, n, 1, beta, ones.data(), 1,
                       matC, std::max<size_t>(1, n), 1.0f, matOut,
                       std::max<size_t>(1, ldc));
  } else if (broadcastHint == BroadcastType::COLUMN) {
    std::vector<float> ones(n, 1.0f);
    cblas::cblas_sgemm(cblas::Layout::RowMajor, cblas::Transpose::None,
                       cblas::Transpose::None, m, n, 1, beta, matC, 1,
                       ones.data(), std::max<size_t>(1, n), 1.0f, matOut,
                       std::max<size_t>(1, ldc));
    ...
    cblas::cblas_sgemm(cblas::Layout::RowMajor, cblas::Transpose::None,
                       cblas::Transpose::None, m, n, 1, beta, ones.data(), 1,
                       bias.data(), std::max<size_t>(1, n), 1.0f, matOut,
                       std::max<size_t>(1, ldc));
    ...
    cblas::cblas_sgemm(
        ...
        m, n, n, beta, matC, std::max<size_t>(1, n), identity.data(),
        std::max<size_t>(1, n), 1.0f, matOut, std::max<size_t>(1, ldc));
  } else {
    NGRAPH_UNREACHABLE("Unsupported broadcast");
  }
}

extern "C" void _mlir_ciface_callback_1_input(void *input, void *output,
                                              void *attrsPtr, OpType type) {
...
extern "C" void _mlir_ciface_callback_2_inputs(void *input0, void *input1,
                                               void *output, void *attrsPtr,
                                               OpType type) {
  ...
  if (type == OpType::MATMUL) {
    ...
    __mlir_mkldnn_maxpoolbackprop(
        unrankedMemRefInput0->rank, unrankedMemRefInput0->memRefDescPtr,
        unrankedMemRefInput1->memRefDescPtr,
        unrankedMemRefOutput->memRefDescPtr, static_cast<opAttrs *>(attrsPtr));
  } else {
    NGRAPH_UNREACHABLE("Unsupported type");
...
extern "C" void _mlir_ciface_callback_3_inputs(void *input0, void *input1,
