...
enum class BroadcastType { NONE, ROW, COLUMN, ROWCOLUMN, ERROR };

// These structs and union are used to pass attributes to callbacks.
template <int N> struct convAttrs {
  bool withRelu;
...
template <int N> struct poolAttrs {
...
enum class AttrsType { INT = 0, CONV1D, CONV2D, CONV3D, POOL2D, POOL3D, GEMM };
