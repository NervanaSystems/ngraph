...
static llvm::cl::opt<bool> clDumpObjectFile(
    "ngraph-dump-mlir-object-file",
    llvm::cl::desc("Dump MLIR JITted-compiled object to file specified with "
                   "-object-filename (<input file>.o by default)."));

static llvm::cl::opt<std::string> clObjectFilename(
    "ngraph-mlir-object-filename",
...
llvm::cl::opt<bool> clEnableBarePtrMemRefLowering(
    "ngraph-bare-ptr-memref-lowering", llvm::cl::init(false),
...
void MLIRCPURuntime::run_internal(const std::vector<MemRefArg> &args,
  ...
  if (!m_engine) {
    auto llvmTransformer = mlir::makeOptimizingTransformer(
        MLIRCPUBackend::mlirOptLevel, /*sizeLevel=*/0,
        MLIRCPUBackend::targetMachine.get());
...
void MLIRCPURuntime::bindArguments(const std::vector<MemRefArg> &args) {
  ...
  NGRAPH_CHECK(
      m_invokeArgs.size() == m_externalTensors->size(),
  ...
  for (size_t i = 0, numArgs = m_invokeArgs.size(); i < numArgs; ++i) {
    if (!clEnableBarePtrMemRefLowering) {
      ...
      for (auto j = 0; j < rank; j++) {
        ...
        memRefArg->shapeAndStrides[rank + j] =
            (*m_externalTensors)[i].m_strides[j];
      }
    } else {
...
void MLIRCPURuntime::execute(bool firstIteration) {
  ...
  if (!clEnableBarePtrMemRefLowering) {
    if (firstIteration) {
      ...
      if (clDumpObjectFile) {
        m_engine->dumpToObjectFile(clObjectFilename.empty()
                                       ? "jitted_mlir.o"
    ...
    auto invocationResult = m_engine->invoke(
        "_mlir_ciface_main", llvm::MutableArrayRef<void *>(m_invokeArgs));
    if (clDumpObjectFile) {
      m_engine->dumpToObjectFile(clObjectFilename.empty()
                                     ? "jitted_mlir.o"
                                     : clObjectFilename.getValue());
    ...
    NGRAPH_CHECK(!invocationResult,
                 "JIT invocation of '_mlir_ciface_main' failed\n");
  } else {
...
StaticMemRef *MLIRCPURuntime::allocateDefaultMemrefDescriptor(size_t rank) {
  ...
  // sizeof(int64_t).
  auto *descriptor = reinterpret_cast<StaticMemRef *>(
      malloc(sizeof(StaticMemRef) + 2 * rank * sizeof(int64_t)));
