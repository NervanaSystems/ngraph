...
void ngraph::runtime::gpu::pass::LSTMFusion::construct_lstm_fprop()
{
    ...
    auto callback = [this,
        ...
        // Determine which is ht_1 and xt. but if both xt and ht_1 have the same shape we need
        // to capture this reliably in the RNN fusion.
        ...
        if (!intermediate_lstm &&
            (is_type<op::Broadcast>(pattern_map[hidden_ht].get_node()) &&
             is_type<op::Constant>(
                 pattern_map[hidden_ht].get_node()->get_input_node_shared_ptr(0))))
        // label input_xt is the input data to the first LSTM
        {
            ...
            lstm = std::make_shared<op::gpu::Rnn>(pattern_map[input_xt],
                 ...
                 (is_type<op::Broadcast>(pattern_map[input_xt].get_node()) &&
                  is_type<op::Constant>(
                      pattern_map[input_xt].get_node()->get_input_node_shared_ptr(0))))
        // label hidden_ht is the input data to the first LSTM
        {
            ...
            lstm = std::make_shared<op::gpu::Rnn>(pattern_map[hidden_ht],
                 ...
                 pattern_map[ct_1].get_node()->get_input_node_shared_ptr(0)->get_instance_id() ==
                     pattern_map[hidden_ht]
                         .get_node()
        ...
        NGRAPH_DEBUG << "End construct_fprop_lstm callback against "
                     << m.get_match_root()->get_name();
        return true;
    };
                     ...
                     bool concat_all = false)
{
    ...
    if (concat_all)
    {
        auto node_labels = m.get_bound_values_for_pattern(rnn_labels[0]);
...
void ngraph::runtime::gpu::pass::RNNFusion::construct_rnn_lstm_fprop()
{
    ...
    auto callback = [this, lstm_node_label, xt, ht_1, params_label, rpattern_ct_1](
        ...
        Output<ngraph::Node> src_layer;
        Output<ngraph::Node> src_iter;

        auto xt_node_array = m.get_bound_values_for_pattern(xt);
        auto hidden_ht_array = m.get_bound_values_for_pattern(ht_1);
        ...
        if (is_type<op::Broadcast>(xt_node_array[xt_node_array.size() - 1].get_node()) &&
            is_type<op::Constant>(
                xt_node_array[xt_node_array.size() - 1].get_node()->get_input_node_shared_ptr(0)))
        ...
        else if (is_type<op::Broadcast>(hidden_ht_array[hidden_ht_array.size() - 1].get_node()) &&
                 is_type<op::Constant>(hidden_ht_array[hidden_ht_array.size() - 1]
                                           .get_node()
                                           ->get_input_node_shared_ptr(0)))
        ...
        RETURN_IF_FALSE(
            src_layer.get_node()->get_arguments().size() == sequence_len ||
                std::dynamic_pointer_cast<op::Parameter>(src_layer.get_node_shared_ptr()),
        ...
        auto lstm_goes = m.get_bound_values_for_pattern(lstm_node_label);
        ...
        for (size_t i = 0; i < lstm_goes.size(); i++)
        {
            ...
            lstm_nodes.push_back(lstm_goes[i].get_node()->get_arguments()[0]);
        }
        ...
        for (size_t index = 0; index < lstm_nodes.size(); index++)
        {
            ...
            for (Input<Node> goes : lstm_nodes[index]->output(0).get_target_inputs())
            {
                ...
                //             NGRAPH_DEBUG << "ht_slice: " <<
                //             ht_slice_per_timestep[index]->get_name()
        ...
        for (auto& node : lstm_goe0_user)
        {
            for (size_t i = 0; i < node->get_input_size(); i++)
            {
                if (map_goe_to_lstm_slices.find(node->get_input_node_shared_ptr(i)) !=
                    map_goe_to_lstm_slices.end())
                {
                    node->input(i).replace_source_output(
                        map_goe_to_lstm_slices[node->get_input_node_shared_ptr(i)]->output(0));
                }
            }
        }
...
static std::shared_ptr<Node>
    compute_multi_layer_rnn_inputs(const std::shared_ptr<pattern::op::Label>& rnn_label,
                                   pattern::RecurrentMatcher& m)
{
    auto node_labels = m.get_bound_values_for_pattern(rnn_label);
    std::reverse(node_labels.begin(), node_labels.end());
    return std::make_shared<op::Concat>(node_labels, 0);
}
...
void ngraph::runtime::gpu::pass::MultiLayerRNNFusion::construct_multi_layer_rnn_fusion_fprop()
{
    ...
    auto callback = [src_layer_label, src_iter_label, params_label, state_iter_label, rnn_ht_label](
        ...
        auto src_layer_nodes = m.get_bound_values_for_pattern(src_layer_label);
        ...
        {
            auto rnn_op =
                std::dynamic_pointer_cast<op::gpu::Rnn>(rnn_goe_input.get_node_shared_ptr());
            if (rnn_op)
            {
        ...
        if (auto src_rnn = std::dynamic_pointer_cast<op::gpu::Rnn>(src_layer.get_node_shared_ptr()))
        {
            RETURN_IF_FALSE(
        ...
        NGRAPH_DEBUG << "End construct_multi_layer_rnn_fusion_fprop callback against "
                     << m.get_match_root()->get_name();
        return true;
    };
