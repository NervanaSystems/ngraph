...
OPTIONS
        ...
        --compare                 compare outputs with tensors from <file>.out
        --save_tensors            save input and output tensors to <file>.in and <file>.out
            ...
            if (!backend.empty())
            {
                ...
                else
                {
                    perf_data = run_benchmark(
                        f, backend, iterations, timing_detail, warmup_iterations, copy_data, compare, save_tensors, model+".in", model+".out");
                }
