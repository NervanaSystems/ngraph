...
from ngraph.utils.input_validation import _check_value, check_valid_attribute, \
    check_valid_attributes, is_positive_value, is_non_negative_value
...
def test_check_valid_attribute():
    ...
    assert check_valid_attribute(
        'TestOp', attr_dict, 'mode', np.str_, required=True)
    assert check_valid_attribute(
