...
TEST(specialize_function, et_static_shape_rank_static_dynamic_subst_val)
{
    ...
    auto plus_node = as_type_ptr<op::Add>(g->get_results().at(0)->get_input_node_shared_ptr(0));
    ASSERT_TRUE(plus_node);
    auto convert_node = as_type_ptr<op::Convert>(plus_node->get_input_node_shared_ptr(1));
    ASSERT_TRUE(convert_node);
    auto const_node = as_type_ptr<op::Constant>(convert_node->get_input_node_shared_ptr(0));
