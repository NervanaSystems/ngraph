...
TEST(specialize_function, share_constants)
{
    ...
    auto mul = std::make_shared<op::Multiply>(p0,
                                              mul_const,
                                              op::AutoBroadcastType::NUMPY);

    auto add_const = op::Constant::create(element::f32, Shape{1, 3, 1, 1}, {4, 5, 6});
    auto add = std::make_shared<op::Multiply>(mul,
                                              add_const,
                                              op::AutoBroadcastType::NUMPY);

    auto f = std::make_shared<Function>(add, ParameterVector{p0});

    auto f_specialized = specialize_function(f,
                                             {element::f32},
                                             {PartialShape{2, 3, 64, 64}},
                                             {nullptr},
                                             true,
                                             true);
...
TEST(specialize_function, share_constants_with_cf)
{
    ...
    auto add = std::make_shared<op::Add>(mul,
                                         add_const,
                                         op::AutoBroadcastType::NUMPY);
