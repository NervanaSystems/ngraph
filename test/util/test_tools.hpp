...
std::vector<std::shared_ptr<ngraph::runtime::Tensor>>
    prepare_and_run(const std::shared_ptr<ngraph::Function>& function,
                    std::shared_ptr<ngraph::Function>& compiled_function,
                    ...
                    const std::string& backend_id)
{
    ...
    compiled_function = handle->get_compiled_function();
...
{
    std::vector<std::vector<T>> emptyargs;
    return prepare_and_run<T, T>(function, compiled_function, args, emptyargs, backend_id);
}
...
{
    std::vector<std::shared_ptr<ngraph::runtime::Tensor>> result_tensors =
        prepare_and_run(function, compiled_function, t1args, t2args, backend_id);
...
{
    std::vector<std::vector<TIN>> emptyargs;
    return execute<TIN, TIN, TOUT>(function, compiled_function, args, emptyargs, backend_id);
}
