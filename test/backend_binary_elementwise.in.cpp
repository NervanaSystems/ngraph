...
NGRAPH_TEST(${BACKEND_NAME}, divide_int32_bfloat16_fail)
{
    Shape shape{2, 2};
...
NGRAPH_TEST(${BACKEND_NAME}, divide_adjoint_stability)
{
    ...
    backend->call_with_validate(handle, {resulta, resultb}, {a, b, c});
...
NGRAPH_TEST(${BACKEND_NAME}, maximum_int32_bfloat16_fail)
{
    Shape shape{2, 2};
    auto A = make_shared<op::Parameter>(element::i32, shape);
    auto B = make_shared<op::Parameter>(element::i32, shape);
