...
TEST(reshape_elimination, dot_transpose_to_dot_w_transpose_args)
{
    ...
    pass_manager.run_passes(func);
    auto gdot = graph->get_input_node_shared_ptr(0);
    ASSERT_TRUE(as_type_ptr<op::Dot>(gdot));
    ASSERT_TRUE(as_type_ptr<op::Reshape>(gdot->get_input_node_shared_ptr(0)));
    ASSERT_TRUE(as_type_ptr<op::Reshape>(gdot->get_input_node_shared_ptr(1)));
    ASSERT_EQ(gdot->get_input_node_shared_ptr(0)->get_input_node_shared_ptr(0), x);
    ASSERT_EQ(gdot->get_input_node_shared_ptr(1)->get_input_node_shared_ptr(0), W);
