...
NGRAPH_TEST(${BACKEND_NAME}, layer_norm_bprop_4d_input)
{
    ...
    EXPECT_TRUE(test::all_close_f(expected_data, read_vector<float>(d_data), 1e-5f, 1e-6f));
    EXPECT_TRUE(test::all_close_f(expected_scale, read_vector<float>(d_scale), 1e-5f, 1e-6f));
    EXPECT_TRUE(test::all_close_f(expected_bias, read_vector<float>(d_bias), 1e-5f, 1e-6f));
}
