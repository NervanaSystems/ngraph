    ...
    const std::vector<T>& expected_values)
{
    ...
    auto strides_op = make_shared<ngraph::op::Parameter>(element::i64, Shape{ strides_values.size() });

    std::vector<T> input_values(shape_size(input_shape));
    std::iota(input_values.begin(), input_values.end(), static_cast<T>(0));

    auto strided_slice = std::make_shared<op::StridedSlice>(arg, begin_op, end_op, strides_op,
        begin_mask, end_mask, new_axis_mask, shrink_axis_mask, ellipsis_mask);

    auto f = std::make_shared<Function>(NodeVector{ strided_slice }, ParameterVector{ arg, begin_op, end_op, strides_op });
...
NGRAPH_TEST(${BACKEND_NAME}, strided_slice_0)
{
    check_success<uint32_t>
        (element::u32,
            ...
            std::vector<int64_t>{0, 0, 0,  0, 0, 0, 0},
