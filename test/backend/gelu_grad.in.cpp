...
NGRAPH_TEST(${BACKEND_NAME}, gelu_grad_f32)
{
    Shape shape{8};
    auto A = make_shared<op::Parameter>(element::f32, shape);
    auto f = make_shared<Function>(make_shared<op::GeluGrad>(A), ParameterVector{A});
    ...
    std::transform(input.begin(), input.end(), input.begin(), [](float x) -> float {
        return static_cast<float>(gelu_grad(x));
    });
...
NGRAPH_TEST(${BACKEND_NAME}, gelu_grad_f64)
{
    Shape shape{8};
    ...
    std::transform(
        input.begin(), input.end(), input.begin(), [](double x) -> double { return gelu_grad(x); });
