...
TEST(builder, numpy_transpose)
{
    ...
    auto transposed = as_type_ptr<op::v0::Reshape>(builder::numpy_transpose(param));
    EXPECT_EQ(Shape({4, 2}), transposed->get_output_shape(0));

    // Multidimensional Transpose
    shape = Shape{2, 4, 8};
    param = make_shared<op::v0::Parameter>(element::f32, shape);
    transposed = as_type_ptr<op::v0::Reshape>(builder::numpy_transpose(param));
    EXPECT_EQ(Shape({8, 4, 2}), transposed->get_output_shape(0));

    // Dimshuffle
    shape = Shape{2, 4, 8};
    param = make_shared<op::v0::Parameter>(element::f32, shape);
    transposed = as_type_ptr<op::v0::Reshape>(builder::numpy_transpose(param, AxisVector{2, 0, 1}));
    EXPECT_EQ(Shape({8, 2, 4}), transposed->get_output_shape(0));

    // Bad Orders
    EXPECT_ANY_THROW(as_type_ptr<op::v0::Reshape>(builder::numpy_transpose(param, AxisVector{2})));
    EXPECT_ANY_THROW(
        as_type_ptr<op::v0::Reshape>(builder::numpy_transpose(param, AxisVector{2, 2, 1})));
}

TEST(builder, tensor_mask)
{
    Shape max_sequence_length{3};
    auto sequence_lengths = make_shared<op::v0::Parameter>(element::u32, max_sequence_length);
