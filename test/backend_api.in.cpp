...
NGRAPH_TEST(${BACKEND_NAME}, create_tensor_2_input)
{
    ...
    vector<float> av = {1, 2, 3, 4};
    vector<float> bv = {5, 6, 7, 8};
    vector<float> rv(shape_size(shape));
    shared_ptr<runtime::Tensor> a = exec->create_input_tensor(0, av.data());
    shared_ptr<runtime::Tensor> b = exec->create_input_tensor(1, bv.data());
    shared_ptr<runtime::Tensor> result = exec->create_output_tensor(0, rv.data());

    exec->call_with_validate({result}, {a, b});
    vector<float> expected = {6, 8, 10, 12};
    EXPECT_TRUE(test::all_close_f(rv, expected, MIN_FLOAT_TOLERANCE_BITS));
}
...
NGRAPH_TEST(${BACKEND_NAME}, create_tensor_2_output)
{
    ...
    shared_ptr<runtime::Tensor> result = exec->create_output_tensor(0, actual.data());
