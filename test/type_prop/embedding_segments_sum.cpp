...
TEST(type_prop, ess)
{
    ...
    auto ess = make_shared<op::v3::EmbeddingSegmentsSum>(
        emb_table, indices, segment_ids, num_segments, per_sample_weights, default_index);
    EXPECT_TRUE(
...
TEST(type_prop, ess_5_args_api)
{
    ...
    auto ess = make_shared<op::v3::EmbeddingSegmentsSum>(
        emb_table, indices, segment_ids, num_segments, per_sample_weights);
...
TEST(type_prop, ess_fail_per_sample_weights_element_type_5_args_api)
{
    auto emb_table = make_shared<op::Parameter>(element::f32, Shape{5, 2});
    auto indices = make_shared<op::Parameter>(element::i64, Shape{4});
    auto segment_ids = make_shared<op::Parameter>(element::i64, Shape{4});
    ...
    try
    {
        auto ess = make_shared<op::v3::EmbeddingSegmentsSum>(
            emb_table, indices, segment_ids, num_segments, per_sample_weights);
        FAIL() << "Invalid per_sample_weights type not detected";
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(),
    ...
    {
        FAIL() << "per_sample_weights type check failed for unexpected reason";
    }
}