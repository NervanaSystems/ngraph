...
TEST(type_prop, reshape_deduce_s2v)
{
    ...
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{}, Shape{1});
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{}, Shape{1, 1});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{}, Shape{1, 1, 1});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{1});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{0}, Shape{});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{1, 1});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{0, 1}, Shape{});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
    ASSERT_EQ(r->get_output_shape(0), (Shape{}));
}
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{1, 1, 1});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{0, 1, 2}, Shape{});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
    ASSERT_EQ(r->get_output_shape(0), (Shape{}));
}
...
TEST(type_prop, reshape_deduce_m2v_01)
{
    ...
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{0, 1}, Shape{12});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{3, 4});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 0}, Shape{12});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
    ASSERT_EQ(r->get_output_shape(0), (Shape{12}));
}
...
TEST(type_prop, reshape_deduce_t2v_012)
{
    ...
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{0, 1, 2}, Shape{60});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
{
    auto param = make_shared<op::v0::Parameter>(element::f32, Shape{3, 4, 5});
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 2, 0}, Shape{60});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
    ASSERT_EQ(r->get_output_shape(0), (Shape{60}));
}
...
TEST(type_prop, reshape_deduce_not_enough_axes)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 0}, Shape{60});
...
TEST(type_prop, reshape_deduce_too_many_axes)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 2, 0, 3}, Shape{60});
        // Should have thrown, so fail if it didn't
...
TEST(type_prop, reshape_deduce_duplicate_axes)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 1, 0}, Shape{60});
        // Should have thrown, so fail if it didn't
        FAIL() << "Too many axes not detected";
    }
    catch (const NodeValidationFailure& error)
...
TEST(type_prop, reshape_deduce_wrong_output_shape)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{1, 2, 0}, Shape{3, 3, 3});
        // Should have thrown, so fail if it didn't
        FAIL() << "Too many axes not detected";
    }
    catch (const NodeValidationFailure& error)
...
TEST(type_prop, reshape_partial_rank_dynamic_axisvector_ok)
{
    ...
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{2, 1, 0, 3}, Shape{3, 1, 8, 2});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
...
TEST(type_prop, reshape_partial_rank_dynamic_axisvector_not_ok)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{2, 1, 0, 4}, Shape{3, 1, 8, 2});
        // Should have thrown, so fail if it didn't
...
TEST(type_prop, reshape_partial_rank_static_dynamic_axisvector_not_ok)
{
    ...
    try
    {
        auto r = make_shared<op::v0::Reshape>(param, AxisVector{2, 1, 0}, Shape{3, 1, 8, 2});
        // Should have thrown, so fail if it didn't
...
TEST(type_prop, reshape_partial_rank_static_dynamic_but_zero_ok)
{
    ...
    auto param = make_shared<op::v0::Parameter>(element::f32, PartialShape::dynamic());
    auto r = make_shared<op::v0::Reshape>(param, AxisVector{2, 1, 0, 3}, Shape{3, 1, 0, 2});
    ASSERT_EQ(r->get_output_element_type(0), element::f32);
    ASSERT_TRUE(r->get_output_partial_shape(0).is_static());
