...
TEST(type_prop, deformable_psroi_pooling_output_shape)
{
    ...
    auto def_psroi_pool = make_shared<op::v1::DeformablePSROIPooling>(input, coords, output_dim, group_size, spatial_scale);
...
TEST(type_prop, deformable_psroi_pooling_invalid_input_rank)
{
    ...
    {
        EXPECT_HAS_SUBSTRING(error.what(), std::string("Feature map input rank must equal to 4 (input rank: 3)"));
    }
    catch (...)
    {
...
TEST(type_prop, deformable_psroi_pooling_invalid_box_coordinates_rank)
{
    ...
    {
        EXPECT_HAS_SUBSTRING(error.what(), std::string("Box coordinates input rank must equal to 2 (input rank: 3)"));
    }
    catch (...)
    {
        FAIL() << "Deduced type check failed for unexpected reason";
...
TEST(type_prop, deformable_psroi_pooling_invalid_offstes_rank)
{
    ...
    try
    {
        auto def_psroi_pool = make_shared<op::v1::DeformablePSROIPooling>(input, coords, offsets, output_dim, group_size, spatial_scale);
        // Should have thrown, so fail if it didn't
        FAIL() << "Offsets input rank not detected";
    }
    catch (const NodeValidationFailure& error)
    {
        EXPECT_HAS_SUBSTRING(error.what(), std::string("Offsets input rank must equal to 4 (input rank: 5)"));
    }
    catch (...)
    {
        FAIL() << "Deduced type check failed for unexpected reason";
