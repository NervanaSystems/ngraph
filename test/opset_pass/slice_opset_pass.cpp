...
TEST(opset_transform, opset1_dyn_slice_upgrade_pass)
{
    ...
    auto slice_v0 = make_shared<op::v0::Slice>(arg,
                                               lower_bounds,
                                               upper_bounds,
                                               strides);
    ...
    auto begin_const = as_type_ptr<op::Constant>(strided_slice_v1->input_value(1).get_node_shared_ptr());
    auto end_const = as_type_ptr<op::Constant>(strided_slice_v1->input_value(2).get_node_shared_ptr());
    auto strides_const = as_type_ptr<op::Constant>(strided_slice_v1->input_value(3).get_node_shared_ptr());
    ...
    EXPECT_EQ(begin_const->get_vector<int64_t>(), vector<int64_t>(lower_bounds.begin(), lower_bounds.end()));
    EXPECT_EQ(end_const->get_vector<int64_t>(), vector<int64_t>(upper_bounds.begin(), upper_bounds.end()));
    EXPECT_EQ(strides_const->get_vector<int64_t>(), vector<int64_t>(strides.begin(), strides.end()));
}
...
TEST(opset_transform, opset1_strided_slice_downgrade_pass)
{
    ...
    auto strided_slice_v1 = make_shared<op::v1::StridedSlice>(data,
                                                              ...
                                                              vector<int64_t>{0, 0, 1, 0},
                                                              vector<int64_t>{1, 0, 0, 0});
