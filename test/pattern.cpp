...
TEST(pattern, zero_const)
{
    ...
    auto f = std::make_shared<Function>(constant, op::ParameterVector{});
    auto manager = runtime::Manager::get("CPU");
    auto external = manager->compile(f);
    auto backend = manager->allocate_backend();
    auto cf = backend->make_call_frame(external);
    shared_ptr<runtime::TensorView> a = backend->make_primary_tensor_view(element::i32, zero_shape);
    shared_ptr<runtime::TensorView> result = backend->make_primary_tensor_view(element::i32, zero_shape);
    shared_ptr<runtime::TensorView> result2 = backend->make_primary_tensor_view(element::i32, zero_shape);
    cf->call({}, {result});
}


TEST(pattern, zero_const2)
{

    Shape zero_shape{0};
    auto A = std::make_shared<op::Parameter>(element::i32, zero_shape);
    auto B = std::make_shared<op::Parameter>(element::f32, zero_shape);
    auto C = std::make_shared<op::Parameter>(element::i32, Shape{1});
    auto D = std::make_shared<op::Parameter>(element::f32, Shape{1,1,0});
    auto weights = std::make_shared<op::Parameter>(element::f32, Shape{1,1,4});

    auto sum = std::make_shared<op::Sum>(A, AxisSet{0});
    std::cout << "sum size = " << vector_to_string(sum->get_shape()) << std::endl;

    auto product = std::make_shared<op::Product>(A, AxisSet{0});
    std::cout << "product size = " << vector_to_string(product->get_shape()) << std::endl;
    ...
    */
    auto convolution = std::make_shared<op::Convolution>(D,weights,Strides{1},Strides{1},CoordinateDiff{2},CoordinateDiff{2});
    std::cout << "convolution size = " << vector_to_string(convolution->get_shape()) << std::endl;

    //auto replace_slice = std::make_shared<op::ReplaceSlice>(A, C, Coordinate{0}, Coordinate{0});
    //std::cout << "replace_slice size = " << vector_to_string(replace_slice->get_shape()) << std::endl;        
...
TEST(pattern, zero_const_conv)
{
    Shape zero_shape{0};
    auto A = std::make_shared<op::Parameter>(element::f32, Shape{1,1,0});
    auto weights = std::make_shared<op::Parameter>(element::f32, Shape{1,1,4});
    auto convolution = std::make_shared<op::Convolution>(A,weights,Strides{1},Strides{1},CoordinateDiff{2},CoordinateDiff{2});
    auto abs_node = std::make_shared<op::Abs>(convolution);
    auto constant = std::make_shared<op::Constant>(element::i32, zero_shape, std::vector<string>{});
    auto f = std::make_shared<Function>(NodeVector{abs_node, constant}, op::ParameterVector{A, weights});
    pass::Manager pass_manager;
...
TEST(pattern, zero_const_avg_pool)
{
    Shape zero_shape{0};
    auto A = std::make_shared<op::Parameter>(element::f32, Shape{1, 1, 0});

    auto avg_pool = std::make_shared<op::AvgPool>(A, Shape{1}, Strides{1}, Shape{2}, Shape{2}, true);
