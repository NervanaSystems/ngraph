...
TEST(nvidiagpu_fusion, rnn_fprop_1_lstm_cell)
{
    ...
    auto rnn_node = make_shared<op::nvidiagpu::Rnn>(src_layer,
                                                    src_iter,
                                                    params,
                                                    state_iter,
                                                    number_of_timesteps,
...
TEST(nvidiagpu_fusion, fuse_lstm_cells)
{
    ...
    auto lstm_ops = get_ops_of_type<op::nvidiagpu::Rnn>(func);
    EXPECT_EQ(lstm_ops.size(), 6);
}

TEST(nvidiagpu_fusion, fuse_2_layer_rnn)
{
    pass::Manager pass_manager;
    pass_manager.register_pass<runtime::nvidiagpu::pass::LSTMFusion>();
    ...
    shared_ptr<Function> func = ngraph::deserialize(ss);
    pass_manager.run_passes(func);
    size_t count = count_ops_of_type<op::nvidiagpu::Rnn>(func);
    auto rnn_ops = get_ops_of_type<op::nvidiagpu::Rnn>(func);
...
TEST(DISABLED_nvidiagpu_fusion, fuse_1_layer_rnn)
{
    pass::Manager pass_manager;
    pass_manager.register_pass<runtime::nvidiagpu::pass::LSTMFusion>();
    pass_manager.register_pass<runtime::nvidiagpu::pass::RNNFusion>();
...
TEST(nvidiagpu_fusion, lstm_analytic)
{
...
TEST(nvidiagpu_fusion, fuse_2_layer_rnn_1lstm_analytic)
{
    auto input_xt = std::make_shared<op::Parameter>(element::f32, Shape{1, 1});
    auto weights_i2h = std::make_shared<op::Parameter>(element::f32, Shape{4, 1});
    auto weights_i2h_reshape =
    ...
    //EXPECT_EQ(1, count_ops_of_type<op::nvidiagpu::Rnn>(f));
...
TEST(nvidiagpu_fusion, rnn_fusion_inter_vs_nvidiagpu_1lstm_cell)
{
    const std::string file_name("mxnet/1_lstm_cell_forward.json");
    auto nvidiagpu_f = make_function(file_name);
...
TEST(DISABLED_nvidiagpu_fusion, rnn_fusion_inter_vs_nvidiagpu_1rnn_layer_3lstm_cell)
{
...
TEST(nvidiagpu_fusion, rnn_fusion_inter_vs_nvidiagpu_2rnn_layer_3lstm_cell)
{
...
TEST(nvidiagpu_fusion, fuse_rnn_across_layer)
{
    pass::Manager pass_manager;
    pass_manager.register_pass<runtime::nvidiagpu::pass::LSTMFusion>();
    pass_manager.register_pass<runtime::nvidiagpu::pass::RNNFusion>();
    ...
    auto rnn_count = count_ops_of_type<op::nvidiagpu::Rnn>(func);
    EXPECT_EQ(ref_rnn_count, rnn_count);
}

TEST(nvidiagpu_fusion, fuse_rnn_across_2layer_1timestep)
{
    ...
    // EXPECT_EQ(1, count_ops_of_type<op::nvidiagpu::Rnn>(nvidiagpu_f));
    for (size_t i = 0; i < nvidiagpu_results.size(); i++)
    {
        EXPECT_TRUE(test::all_close(nvidiagpu_results.at(1), int_results.at(1), 1.0e-4f, 1.0e-4f));
    }
}
