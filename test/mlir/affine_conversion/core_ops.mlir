...
// CHECK-DAG: #[[MAP1:[a-zA-Z0-9]+]] = (d0, d1) -> (d0 * 4 + d1)
...
//
// CHECK:       %[[T3:[0-9]+]] = alloc() : memref<24xi8>
// CHECK-NEXT:  %[[T4:[0-9]+]] = std.view %[[T3]][][] : memref<24xi8> to memref<3x2xf32, #[[MAP0]]>
// CHECK:       affine.store %{{[0-9]+}}, %[[T4]][%{{.*}}, %{{.*}}] : memref<3x2xf32, #[[MAP0]]>
//
// CHECK:       %[[T5:[0-9]+]] = alloc() : memref<48xi8>
// CHECK-NEXT:  %[[T6:[0-9]+]] = std.view %[[T5]][][] : memref<48xi8> to memref<3x4xf32, #[[MAP1]]>
// CHECK:       affine.store %{{[0-9]+}}, %[[T6]][%{{.*}}, %{{.*}}] : memref<3x4xf32, #[[MAP1]]>

func @add(%arg0: !ng.tensor<3x2xf32>, %arg1: !ng.tensor<3x2xf32>) -> !ng.tensor<3x2xf32> {
  %0 = "ng.add"(%arg0, %arg1) : (!ng.tensor<3x2xf32>, !ng.tensor<3x2xf32>) -> !ng.tensor<3x2xf32>
  %1 = "ng.add"(%0, %0) : (!ng.tensor<3x2xf32>, !ng.tensor<3x2xf32>) -> !ng.tensor<3x2xf32>
  %2 = "ng.concat"(%0, %1) {concatenation_axis=1 : i64} : (!ng.tensor<3x2xf32>, !ng.tensor<3x2xf32>) -> !ng.tensor<3x4xf32>
  %3 = "ng.add"(%2, %2) : (!ng.tensor<3x4xf32>, !ng.tensor<3x4xf32>) -> !ng.tensor<3x4xf32>
  "ng.return"(%3) : (!ng.tensor<3x4xf32>) -> ()
}
