...
TEST(type_prop_layers, region_yolo1)
{
    ...
    auto op = make_shared<op::RegionYolo>(
        inputs, 0, 0, 0, true, std::vector<int64_t>{}, std::vector<float>{}, 0, 1);
...
TEST(type_prop_layers, region_yolo2)
{
    auto inputs = make_shared<op::Parameter>(element::f32, Shape{1, 125, 13, 13});
    auto op = make_shared<op::RegionYolo>(
        inputs, 0, 0, 0, true, std::vector<int64_t>{}, std::vector<float>{}, 0, 2);
    ASSERT_EQ(op->get_shape(), (Shape{1 * 125 * 13, 13}));
}
...
TEST(type_prop_layers, region_yolo3)
{
    auto inputs = make_shared<op::Parameter>(element::f32, Shape{1, 125, 13, 13});
    auto op = make_shared<op::RegionYolo>(
        inputs, 4, 80, 1, false, std::vector<int64_t>{6, 7, 8}, std::vector<float>{}, 0, -1);
