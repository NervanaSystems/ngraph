...
{
    auto input = make_shared<op::v0::Parameter>(element::f32, Shape{88, 2, 48, 1});
    auto seq_len = make_shared<op::v0::Parameter>(element::f32, Shape{88, 2});
    auto op = make_shared<op::v0::CTCGreedyDecoder>(input, seq_len, false);
    ASSERT_EQ(op->get_output_shape(0), (Shape{2, 88, 1, 1}));
}

TEST(type_prop_layers, detection_output)
{
    auto box_logits = make_shared<op::v0::Parameter>(element::f32, Shape{4, 1, 5, 5});
    auto class_preds = make_shared<op::v0::Parameter>(element::f32, Shape{2, 1, 4, 5});
    auto proposals = make_shared<op::v0::Parameter>(element::f32, Shape{2, 1, 4, 5});
    auto aux_class_preds = make_shared<op::v0::Parameter>(element::f32, Shape{2, 1, 4, 5});
    auto aux_box_preds = make_shared<op::v0::Parameter>(element::f32, Shape{2, 1, 4, 5});
    ...
    ASSERT_EQ(op->get_output_shape(0), (Shape{1, 1, 800, 7}));
}

TEST(type_prop_layers, interpolate)
{
    auto image = make_shared<op::v0::Parameter>(element::f32, Shape{2, 2, 33, 65});
    auto dyn_output_shape = make_shared<op::v0::Parameter>(element::i64, Shape{2});
    ...
    ASSERT_EQ(op->get_output_shape(0), (Shape{2, 2, 15, 30}));
...
TEST(type_prop_layers, prior_box1)
{
    ...
    auto layer_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {32, 32});
    auto image_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {300, 300});
    auto pb = make_shared<op::v0::PriorBox>(layer_shape, image_shape, attrs);
    ASSERT_EQ(pb->get_output_shape(0), (Shape{2, 20480}));
}
...
TEST(type_prop_layers, prior_box2)
{
    ...
    auto image_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {300, 300});
    auto pb = make_shared<op::v0::PriorBox>(layer_shape, image_shape, attrs);
    ASSERT_EQ(pb->get_output_shape(0), (Shape{2, 32768}));
}
...
TEST(type_prop_layers, prior_box3)
{
    ...
    auto image_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {300, 300});
    auto pb = make_shared<op::v0::PriorBox>(layer_shape, image_shape, attrs);
    ASSERT_EQ(pb->get_output_shape(0), (Shape{2, 16}));
}
...
TEST(type_prop_layers, prior_box_clustered)
{
    ...
    auto layer_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {19, 19});
    auto image_shape = op::v0::Constant::create<int64_t>(element::i64, Shape{2}, {300, 300});
    auto pbc = make_shared<op::v0::PriorBoxClustered>(layer_shape, image_shape, attrs);
    // Output shape - 4 * 19 * 19 * 3 (attrs.widths.size())
    ASSERT_EQ(pbc->get_output_shape(0), (Shape{2, 4332}));
}
...
TEST(type_prop_layers, proposal)
{
    ...
    auto class_probs = make_shared<op::v0::Parameter>(element::f32, Shape{batch_size, 12, 34, 62});
    auto class_logits = make_shared<op::v0::Parameter>(element::f32, Shape{batch_size, 24, 34, 62});
    auto image_shape = make_shared<op::v0::Parameter>(element::f32, Shape{3});
    auto op = make_shared<op::v0::Proposal>(class_probs, class_logits, image_shape, attrs);
...
TEST(type_prop_layers, region_yolo1)
{
    ...
    ASSERT_EQ(op->get_output_shape(0), (Shape{1 * 125, 13, 13}));
}
...
{
    auto inputs = make_shared<op::v0::Parameter>(element::f32, Shape{1, 125, 13, 13});
    auto op = make_shared<op::v0::RegionYolo>(
        inputs, 4, 80, 1, false, std::vector<int64_t>{6, 7, 8}, 0, -1);
    ASSERT_EQ(op->get_output_shape(0), (Shape{1, (80 + 4 + 1) * 3, 13, 13}));
}

TEST(type_prop_layers, reorg_yolo)
{
    auto inputs = make_shared<op::v0::Parameter>(element::f32, Shape{2, 24, 34, 62});
    auto op = make_shared<op::v0::ReorgYolo>(inputs, Strides{2});
    ASSERT_EQ(op->get_output_shape(0), (Shape{2, 96, 17, 31}));
}

TEST(type_prop_layers, psroi_pooling)
{
    auto inputs = make_shared<op::v0::Parameter>(element::f32, Shape{1, 3, 4, 5});
    auto coords = make_shared<op::v0::Parameter>(element::f32, Shape{150, 5});
    auto op = make_shared<op::v0::PSROIPooling>(inputs, coords, 2, 6, 0.0625, 0, 0, "Avg");
    ASSERT_EQ(op->get_output_shape(0), (Shape{150, 2, 6, 6}));
}

TEST(type_prop_layers, roi_pooling)
{
    auto inputs = make_shared<op::v0::Parameter>(element::f32, Shape{2, 3, 4, 5});
    auto coords = make_shared<op::v0::Parameter>(element::f32, Shape{150, 5});
    auto op = make_shared<op::v0::ROIPooling>(inputs, coords, Shape{6, 6}, 0.0625, "Max");
    ASSERT_EQ(op->get_output_shape(0), (Shape{150, 3, 6, 6}));
}
