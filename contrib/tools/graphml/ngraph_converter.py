#
#  Simple converter to convert  ngraph JSON format to 
#  GraphML understood by FGA
#
#  Author: Vasanth Tovinkere
#  Date: 2/5/2018
#
#  Usage:- python.py <file>.json > file.graphml
#
import json
...
def prepare_header(name):
    ...
    print("<graphml name=\"{}\" version =\"1.17041\">".format(name))
...
def output_required_properties(indent, name, operation, description):
    indentation = " "
    i = 1
    while (i < indent):
        indentation += " "
...
def output_node(indent, name, operation, description, has_inputs, inputs, has_outputs, outputs, data_type):
    ...
    indent += 3
    output_required_properties(indent, name, operation, description)
...
with open('test.json', 'r') as f:
    data = json.load(f)
#
#  Extract the graphml file name
#
graphml_name  = data[0]['name']
graphml_name += ".graphml"
#
#  Write the GraphML headers along with the
#  required keys for each node
#
prepare_header(graphml_name)
#
...
for i, graph in enumerate(data):
    ...
    for node in operations_list:
        ...
        #
        if 'element_type' in node:
            data_type = node['element_type']
        else:
            data_type = "int"
        if 'shape' in node:
